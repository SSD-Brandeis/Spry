...
class BitvectorSuffix : public Bitvector {
 public:
  ...
  virtual ~BitvectorSuffix() {}

  virtual BitvectorSuffix* clone() const {
    BitvectorSuffix* copy = new BitvectorSuffix();
    copy->num_bits_ = num_bits_;
    copy->type_ = type_;
    copy->hash_suffix_len_ = hash_suffix_len_;
    copy->real_suffix_len_ = real_suffix_len_;
    if (bits_) {
      copy->bits_ = new word_t[numWords()];
      memcpy(copy->bits_, bits_, bitsSize());
    }
    return copy;
  }

  BitvectorSuffix(const SuffixType type, const level_t hash_suffix_len,
  ...
  static word_t constructRealSuffix(const std::string& key, const level_t level,
                                    const level_t len) {
    if (key.length() < level || ((key.length() - level) * 8) < len) return 0;
    word_t suffix = 0;
  ...
  static word_t constructMixedSuffix(const std::string& key,
                                     const level_t hash_len,
                                     const level_t real_level,
  ...
  static word_t extractHashSuffix(const word_t suffix,
                                  const level_t real_suffix_len) {
    return (suffix >> real_suffix_len);
  }

  static word_t extractRealSuffix(const word_t suffix,
                                  const level_t real_suffix_len) {
  ...
  SuffixType getType() const { return type_; }

  level_t getSuffixLen() const { return hash_suffix_len_ + real_suffix_len_; }

  level_t getHashSuffixLen() const { return hash_suffix_len_; }

  level_t getRealSuffixLen() const { return real_suffix_len_; }

  position_t serializedSize() const {
    position_t size = sizeof(num_bits_) + sizeof(type_) +
                      sizeof(hash_suffix_len_) + sizeof(real_suffix_len_) +
                      bitsSize();
    sizeAlign(size);
    return size;
  }

  position_t size() const { return (sizeof(BitvectorSuffix) + bitsSize()); }

  word_t read(const position_t idx) const;
  word_t readReal(const position_t idx) const;
  bool checkEquality(const position_t idx, const std::string& key,
                     const level_t level) const;

  // Compare stored suffix to querying suffix.
  // kReal suffix type only.
  int compare(const position_t idx, const std::string& key,
              const level_t level) const;
