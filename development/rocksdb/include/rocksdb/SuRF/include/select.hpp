...
class BitvectorSelect : public Bitvector {
 public:
  BitvectorSelect()
      : sample_interval_(0), num_ones_(0), select_lut_(nullptr) {};
  ...
  virtual BitvectorSelect* clone() const {
    ...
    copy->sample_interval_ = sample_interval_;
    copy->num_ones_ = num_ones_;
    if (bits_) {
      copy->bits_ = new word_t[numWords()];
      memcpy(copy->bits_, bits_, bitsSize());
    }
    if (select_lut_) {
      position_t num_samples = num_ones_ / sample_interval_ + 1;
      copy->select_lut_ = new position_t[num_samples];
      memcpy(copy->select_lut_, select_lut_, selectLutSize());
    }
    return copy;
  }
