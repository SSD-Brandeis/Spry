...
// YCHUANG_ADDED START
    ...
    std::cout << "\t\t\t" << "Sparse level: " << level
              << " num_items_per_level: " << num_items_per_level[level] << "\n";
    std::cout << "\t\t\t";
  ...
  if (builder->getSuffixType() == kNone) {
    ...
    suffixes_ =
        new BitvectorSuffix(builder->getSuffixType(), hash_suffix_len,
                            real_suffix_len, builder->getSuffixes(),
                            num_suffix_bits_per_level, start_level_, height_);
  }

  // YCHUANG_ADDED START
  left_parentheses_ =
      new Bitvector(builder->getLeftParentheses(),
                    builder->getLeftParenthesesCounts(), start_level_, height_);
  right_parentheses_ = new Bitvector(builder->getRightParentheses(),
                                     builder->getRightParenthesesCounts(),
                                     start_level_, height_);
// YCHUANG_ADDED END
...
bool LoudsSparse::lookupKey(const std::string& key,
  ...
  for (level = start_level_; level < key.length(); level++) {
    ...
    if (!child_indicator_bits_->readBit(pos)) {
      bool termination_rst =
          suffixes_->checkEquality(getSuffixPos(pos), key, level + 1);
...
// YCHUANG_ADDED START
bool LoudsSparse::moveToNextCommonPrefixKey(
  ...
  for (level = start_level_; level < key.length(); level++) {
    ...
    // TO CHECK
    // pos search is unpdated within the search function, however there's a bug
    // in the search function, moving pos_search unequally position of
    // pos_search will be moved by search function (not consistent)
    if (!labels_->search((label_t)key[level], pos_search, node_size)) {
#ifdef DEBUG_LOUDS_SPARSE
      std::cout << " sparse diverge " << " level: " << level << " key:" << key
                << " " << __FILE__ << ":" << __LINE__ << " " << __func__
                << std::endl;
      std::cout << " sparse diverge " << " pos: " << pos
                << " node_label:" << labels_->read(pos)
                << " node_size:" << node_size << " key[level]:" << key[level]
                << " " << __FILE__ << ":" << __LINE__ << " " << __func__
                << std::endl;
#endif  // DEBUG_LOUDS_SPARSE

      // if there's a complete prefix key fully matches the prefix of the search
      // key std::cout << "iter.key_len_ = " << iter.key_len_ << "
      // max_matching_full_prefix_key_level = " <<
      // max_matching_full_prefix_key_level << " " << __FILE__ << ":" <<
      // __LINE__ << " " << __FUNCTION__ << std::endl;
      if (flag_direct_return_if_found_key_end_with_same_prefix == true &&
          max_matching_full_prefix_key_level <
              ((level_t)1e8)) {  // For SuRF_Base in SuRF_RDF (yucheng)
        iter.is_valid_ = true;
    ...
    // // std::cout << "level = " << level << ", (*labels_)[" << pos << "]  = "
    // << (*labels_)[pos] << " node_size = " << nodeSize(pos) << "
    // getFirstLabelPos(node_num) = " <<  getFirstLabelPos(node_num) << " " <<
    ...
    if (!child_indicator_bits_->readBit(pos)) {
#ifdef DEBUG_LOUDS_SPARSE
      std::cout << "sparse trie branch terminates" << " level:" << level
                << " key:" << key << " " << __FILE__ << ":" << __LINE__ << " "
                << __func__ << std::endl;
#endif  // DEBUG_LOUDS_SPARSE

      // if it fully matches the prefix and doesn't have any successor
      if (flag_direct_return_if_found_key_end_with_same_prefix ==
          true) {  // For SuRF_Base in SuRF_RDF (yucheng)
        iter.is_valid_ = true;
        iter.append(key[level], pos);
        return could_be_fp_;
      }
      ...
      if (level == key.size() - 1) {  // same as the key
        ...
        // if there's a complete prefix key fully matches the prefix of the
        // search key
        if (flag_direct_return_if_found_key_end_with_same_prefix == true &&
            max_matching_full_prefix_key_level <
                ((level_t)1e8)) {  // For SuRF_Base in SuRF_RDF (yucheng)
        ...
        // moveToLeftInNextSubtrie(pos_search, node_size-(pos_search-pos),
        // key[level], iter);
        // cannot set to valid because when sparse part reaches to the end
        // (is_valid = False), we have to go back to the dense part (move it 1
        // node forward) to continue the search
        // iter.is_valid_ = true;
    ...
    // std::cout << "next_pos = " << pos << " " << __FILE__ << ":" << __LINE__
    // << " " << std::endl; log the complete prefix key that ends with the same
    // prefix of the search key
    if (flag_direct_return_if_found_key_end_with_same_prefix == true
        // && child_indicator_bits_->readBit(pos)
        && nodeSize(pos) > 1  // if there's only 1 255 as its child, it cannot
                              // be the kTerminator
        && ((*labels_)[pos] ==
            kTerminator)  // kTerminator can only exist as the first child key
    ) {
...
position_t LoudsSparse::appendToPosList(std::vector<position_t>& pos_list,
                                        ...
                                        const level_t level, const bool isLeft,
                                        bool& done) const {
  position_t pos = getFirstLabelPos(node_num);
...
void LoudsSparse::extendPosList(std::vector<position_t>& left_pos_list,
  ...
  if (start_depth > right_pos_list.size()) start_depth = right_pos_list.size();
  if (start_depth == 0) {
    if (left_pos_list.size() == 0)
      left_pos =
          appendToPosList(left_pos_list, left_in_node_num, 0, true, left_done);
    if (right_pos_list.size() == 0)
      right_pos = appendToPosList(right_pos_list, right_in_node_num, 0, false,
                                  right_done);
    start_depth++;
  }
  ...
  for (level_t i = start_depth; i < (height_ - start_level_); i++) {
    ...
    if (!left_done && left_pos_list.size() <= i) {
      ...
      if (!child_indicator_bits_->readBit(left_pos)) left_node_num++;
      left_pos =
          appendToPosList(left_pos_list, left_node_num, i, true, left_done);
    }
    if (!right_done && right_pos_list.size() <= i) {
      right_node_num = getChildNodeNum(right_pos);
      if (!child_indicator_bits_->readBit(right_pos)) right_node_num++;
      right_pos =
          appendToPosList(right_pos_list, right_node_num, i, false, right_done);
    }
  }
}
...
uint64_t LoudsSparse::approxCount(const LoudsSparse::Iter* iter_left,
  ...
  extendPosList(left_pos_list, right_pos_list, in_node_num_left,
                in_node_num_right);
  ...
  for (level_t i = 0; i < search_depth; i++) {
    ...
    if (right_pos == kMaxPos) right_pos = level_cuts_[start_level_ + i] + 1;
    ...
    if (left_pos < right_pos) {
      ...
      if (child_indicator_bits_->readBit(right_pos)) num_leafs++;
      if (child_indicator_bits_->readBit(left_pos)) num_leafs--;
      if (i == ori_left_len - 1) num_leafs--;
      count += num_leafs;
    }
  }
  return count;
...
uint64_t LoudsSparse::getMemoryUsageInBitsSelf() const {
  ...
  if (_surf_env->getShowSurfCompactionInfo()) {
    std::cout << "@surf sparse getMemoryUsageInBitsSelf start-------------"
              << std::endl
              << "(sizeof(this): " << sizeof(this) * 8 << " B "
              << ", labels_->getNumBytes() * 8: " << labels_->getNumBytes() * 8
              << " bits "
              << ", child_indicator_bits_->numBits(): "
              << child_indicator_bits_->numBits() << " bits, "
              << ", child_indicator_bits_->rankLutSize(): "
              << child_indicator_bits_->rankLutSize() << " bits "
              << ", louds_bits_->numBits(): " << louds_bits_->numBits()
              << " bits "
              << ", suffixes_->numBits(): " << suffixes_->numBits() << " bits "
              << ", left_parentheses_->numBits(): "
              << left_parentheses_->numBits() << " bits "
              << ", right_parentheses_->numBits(): "
              << right_parentheses_->numBits() << " bits)"
              << " " << __FILE__ << ":" << __LINE__ << " " << __func__
              << std::endl;
  }
  ...
  return (labels_->getNumBytes() * 8 + child_indicator_bits_->numBits() +
          child_indicator_bits_->rankLutSize() + louds_bits_->numBits() +
          suffixes_->numBits()
          // YCHUANG_ADDED START
          + left_parentheses_->numBits() + right_parentheses_->numBits()
          // YCHUANG_ADDED END
  );
}
...
void LoudsSparse::moveToLeftInNextSubtrie(position_t pos,
  ...
  // searchGreaterThan moves the pos to the greater node, however, node_size is
  // not changed
  if (!labels_->searchGreaterThan(label, pos, node_size)) {
    ...
    std::cout << "sparse move to left most key" << " label_read:" << label_read
              << " " << __FILE__ << ":" << __LINE__ << " " << __func__
              << std::endl;
#endif  // DEBUG_LOUDS_SPARSE
...
int LoudsSparse::Iter::getSuffix(word_t* suffix) const {
  if ((trie_->suffixes_->getType() == kReal) ||
      (trie_->suffixes_->getType() == kMixed)) {
    position_t suffix_pos = trie_->getSuffixPos(pos_in_trie_[key_len_ - 1]);
...
void LoudsSparse::Iter::operator++(int) {
  ...
  while (pos >= trie_->louds_bits_->numBits() ||
    ...
    if (key_len_ == 0) {
      is_valid_ = false;
#ifdef DEBUG_LOUDS_SPARSE
      std::cout << "sparse ++ " << " is_valid_ = false "
                << "key_len_:" << key_len_ << " " << __FILE__ << ":" << __LINE__
                << " " << __func__ << std::endl;
#endif  // DEBUG_LOUDS_SPARSE
      return;
    }
