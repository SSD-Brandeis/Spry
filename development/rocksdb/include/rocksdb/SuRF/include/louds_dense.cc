...
LoudsDense::LoudsDense(const SuRFBuilder* builder) {
  ...
  child_indicator_bitmaps_ = new BitvectorRank(
      ...
      num_bits_per_level, 0, height_);
  prefixkey_indicator_bits_ = new BitvectorRank(
      kRankBasicBlockSize, builder->getPrefixkeyIndicatorBits(),
  ...
  if (builder->getSuffixType() == kNone) {
    ...
    for (level_t level = 0; level < height_; level++)
      num_suffix_bits_per_level.push_back(builder->getSuffixCounts()[level] *
                                          suffix_len);
    suffixes_ = new BitvectorSuffix(builder->getSuffixType(), hash_suffix_len,
  ...
  left_parentheses_ =
      new Bitvector(builder->getLeftParentheses(),
                    builder->getLeftParenthesesCounts(), 0, height_);
  right_parentheses_ =
      new Bitvector(builder->getRightParentheses(),
                    builder->getRightParenthesesCounts(), 0, height_);
// YCHUANG_ADDED END
...
// YCHUANG_ADDED START
  ...
  for (int i = 0; i < child_indicator_bitmaps_->numBits();
       i += kRankBasicBlockSize) {
    for (int j = 0; j < kRankBasicBlockSize; j++) {
...
// YCHUANG ADDED START
bool LoudsDense::moveToNextCommonPrefixKey(
  ...
  for (level_t level = 0; level < height_; level++) {
    ...
    if (level >= key.length()) {  // if run out of searchKey bytes
      ...
      std::cout << "\t\t *** " << "pos: " << pos << " "
                << " nextPos: " << getNextPos(pos - 1) << " " << __FILE__ << ":"
    ...
    // log the complete prefix key that ends with the same prefix of the search
    // key
    if (flag_direct_return_if_found_key_end_with_same_prefix == true &&
  ...
  if (flag_direct_return_if_found_key_end_with_same_prefix == true &&
    ...
    // TODO: needs to take in to the consideration of the maximum matching may
    // exist in the Sparse part or not,  unimplemented which means the
    // max_matching_full_prefix_key_level in the louds_dense shall be recorded
    // if the maximum prefix matching key exists in Sparse Part, then use the
    // one in the sparse part, Otherwise use the maximum prefix matching key in
    // the Dense Part, if the height of the Dense part > 0
    assert(false);
    return could_be_fp_;
  }
...
void LoudsDense::extendPosList(std::vector<position_t>& pos_list,
  ...
  for (level_t i = pos_list.size(); i < height_; i++) {
    node_num = getChildNodeNum(pos);
    if (!child_indicator_bitmaps_->readBit(pos)) node_num++;
    pos = (node_num * kNodeFanout);
  ...
  if (pos == kMaxPos) {
    for (level_t i = pos_list.size(); i < height_; i++) pos_list.push_back(pos);
    out_node_num = pos;
  } else {
    out_node_num = getChildNodeNum(pos);
    if (!child_indicator_bitmaps_->readBit(pos)) out_node_num++;
  }
}
...
uint64_t LoudsDense::approxCount(const LoudsDense::Iter* iter_left,
  ...
  for (level_t i = 0; i < height_; i++) {
    ...
    if (left_pos < right_pos) {
      ...
      if (right_pos == level_cuts_[height_ - 1]) rank_right_label++;
      ...
      if (child_indicator_bitmaps_->readBit(right_pos)) num_leafs++;
      if (child_indicator_bitmaps_->readBit(left_pos)) num_leafs--;
      // offcount in prefix keys
      if (i >= ori_right_len && has_prefix_key_right) num_leafs--;
      if (i >= ori_left_len && has_prefix_key_left) num_leafs++;
...
uint64_t LoudsDense::serializedSize() const {
  uint64_t size = sizeof(height_) + (sizeof(position_t) * height_);
  sizeAlign(size);
...
uint64_t LoudsDense::getMemoryUsage() const {
  ...
  return (sizeof(LoudsDense) + label_bitmaps_->size() +
          ...
          suffixes_->size() + left_parentheses_->size() +
...
uint64_t LoudsDense::getMemoryUsageInBitsSelf() const {
  ...
  if (_surf_env->getShowSurfCompactionInfo()) {
    std::cout << "@surf dense getMemoryUsageInBitsSelf start-------------"
              << std::endl
              << " (sizeof(LoudsDense) = " << sizeof(LoudsDense) << " B"
              << ", label_bits = " << label_bitmaps_->numBits() << " bits"
              << ", label_rankLutSize = " << label_bitmaps_->rankLutSize()
              << " bits"
              << ", child_indicator_bits = "
              << child_indicator_bitmaps_->numBits() << " bits"
              << ", child_indicator_rankLutSize = "
              << child_indicator_bitmaps_->rankLutSize() << " bits"
              << ", prefixkey_bits = " << prefixkey_indicator_bits_->numBits()
              << " bits"
              << ", prefixkey_rankLutSize = "
              << prefixkey_indicator_bits_->rankLutSize() << " bits"
              << ", suffixes_bits = " << suffixes_->numBits() << " bits"
              << ", left_parentheses_bits = " << left_parentheses_->numBits()
              << " bits"
              << ", right_parentheses_bits = " << right_parentheses_->numBits()
              << " bits )"
  ...
  if (label_bitmaps_ == nullptr) {
    return 0;
  }

  return (label_bitmaps_->numBits() + label_bitmaps_->rankLutSize() +
          child_indicator_bitmaps_->numBits() +
          child_indicator_bitmaps_->rankLutSize() +
          prefixkey_indicator_bits_->numBits() +
          prefixkey_indicator_bits_->rankLutSize() + suffixes_->numBits() +
...
int LoudsDense::Iter::compare(const std::string& key) const {
  ...
  if (isComplete()) {
    position_t suffix_pos =
        trie_->getSuffixPos(pos_in_trie_[key_len_ - 1], is_at_prefix_key_);
