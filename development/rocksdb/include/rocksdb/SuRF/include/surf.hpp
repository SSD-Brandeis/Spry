...
class SuRF_Env {
 public:
  ...
  void setFlagAllowRangeBoundaryOverlapped(
      bool flag_allow_range_boundary_overlapped) {
    surf__flag_allow_range_boundary_overlapped =
        flag_allow_range_boundary_overlapped;
    return;
  }

  int getSuRFKeyLenInBytes() const { return surf__key_len_in_bytes; }
  uint32_t getSuRFHashSuffixLen() const { return surf__hash_suffix_len; }
  uint32_t getSuRFRealSuffixLen() const { return surf__real_suffix_len; }
  bool getSuRFIncludeDense() const { return surf__include_dense; }
  uint32_t getSuRFSparseDenseRatio() const { return surf__sparse_dense_ratio; }
  bool getFlagBypassIfSameKey() const {
    if (surf__flag_bypass_if_same_key !=
        surf__flag_allow_range_boundary_overlapped) {
      assert(surf__flag_bypass_if_same_key ==
             surf__flag_allow_range_boundary_overlapped);
      std::cerr << "Error: flag_bypass_if_same_key and "
                   "flag_allow_range_boundary_overlapped are not consistent"
  ...
  void clearFlagKeyMayDeleted() { flag_key_may_deleted = false; }
  // End: used during Get()
  ...
  bool getShowSurfCompactionInfo() { return show_surf_compaction_info; }

  void setFlagUseSuRFBase(bool flag) { use_surf_base = flag; }
  bool getFlagUseSuRFBase() { return use_surf_base; }
...
class SuRF_Utils {
 public:
  // static std::string encode_digit_string_to_byte_string(std::string
  // digit_string);
  static std::string encode_digit_string_to_byte_string(
      const std::string& digit_string) {
    uint32_t base = 256;
    ...
    // std::cout << "input digit_string = " << digit_string << ", encoded_str =
  ...
  static std::string decode_byte_string_to_digit_string(
  ...
  static std::string extend_string_to_length(std::string str, uint32_t len,
                                             char c = (char)0) {
  ...
  // Function to truncate based on the first k_diff differing characters between
  // two strings
  static std::string truncateToFirstKDifference(const std::string& str1,
    ...
    // Iterate over both strings and count differences until k_diff differences
    // are found
    while (i < minLen) {
      if (str1[i] != str2[i]) {
        diff_count++;
    ...
    int parse_len = str1.size() < (minLen + k_diff - diff_count)
                        ? str1.size()
                        : (minLen + k_diff - diff_count);
    return str1.substr(0, parse_len);
    // // Return the substring from str1 up to i+1
    // return str1.substr(0, i + 1);
  }

  // Process the list of words to truncate based on k_diff differences from
  // previous word
  static std::vector<std::string> processStringsToFirstKDifference(
      std::vector<std::string>& vec, int k_diff = 1) {
    // Sort the vector of strings
    std::sort(vec.begin(), vec.end());

    // std::cout << "@ processStringsToFirstKDifference " << __FILE__ << ":" <<
    // __LINE__ << " " << __FUNCTION__ << std::endl; for(uint32_t i = 0; i <
    // vec.size(); i++){
    //     if(surf::SuRF_Env::getInstance()->getFlagSurfUseCondensedDigitKey()
    //     == true){
    //         auto ks =
    //         surf::SuRF_Utils::decode_byte_string_to_digit_string(vec[i]);
    //         std::cout << "key =  " << ks << " " << __FILE__ << ":" <<
    //         __LINE__ << " " << __FUNCTION__ << std::endl;
    //     }else{
    //         std::cout << "key =  " << vec[i] << " " << __FILE__ << ":" <<
    //         __LINE__ << " " << __FUNCTION__ << std::endl;
    //     }
    // }
    ...
    if (vec.size() <= 1) {
      ...
      // vec[0] = truncateToFirstKDifference("", vec[0], k_diff);  // Return the
      // vector as is if it contains 1 or no elements
      vec[0] = truncateToFirstKDifference(
          vec[0], " ",
          k_diff);  // Return the vector as is if it contains 1 or no elements
      return vec;
    }
...
class SuRF_RDF {
 public:
  ...
  SuRF_RDF& operator=(const SuRF_RDF&) = delete;
  ...
  void directMoveFileToLevel(uint64_t fd, uint32_t src_level,
                             uint32_t dst_level);

  std::vector<pss> getRangeTombstonesAtLevelOfFd(
  ...
  void insertRangesAtLevelOfFd(uint32_t level, uint64_t fd,
                               std::vector<pss>& ranges,
                               bool flag_allow_boundary_overlapped);

  void insertRangesWithPointKeysAtLevelOfFd(
  ...
  std::unordered_map<uint64_t, std::pair<int, SuRF*>> getLevelFileRDFAtIthLevel(
      int level);

  void RemoveSuRF(std::vector<uint32_t>& src_level_list,
                  std::vector<std::vector<uint64_t>>& src_fd_list2d);

  std::vector<pss> gatherSortedRangeTombstonesAndRemoveSuRF(
  ...
  void shiftRDFWithPointKeysToOutputLevel(
  ...
  static vpss mergeRanges(vpss ranges_1, vpss ranges_2,
                          bool allow_boundary_overlap_not_merged = false);
  ...
  bool isEntryAliveAtLevelOfFd(level_t level, uint64_t fd, std::string key,
                               bool flag_bypass_if_same_key);

  // bool isEntryAlive(level_t level, std::string key, bool
  // flag_bypass_if_same_key);

  // bool isEntryAlive(level_t level, std::string key, uint64_t fd, bool
  // flag_bypass_if_same_key);
 ...
 private:
  ...
  std::vector<int>
      memory_usage_in_RDF_log;  // for level > 0, number of ranges in RDF
  bool flag_key_may_deleted = false;
  uint32_t key_search_count_kmd =
      0;  // kmd = key may deleted, just for functionality verification
...
class SuRF {
 public:
  class Iter {
   public:
    ...
    LoudsSparse::Iter* getSparseIter() { return &sparse_iter_; }
    // YCHUANG ADDED END
 ...
 public:
  ...
  SuRF(const std::vector<std::string>& keys, const SuffixType suffix_type,
    ...
    create(keys, kIncludeDense, kSparseDenseRatio, suffix_type, hash_suffix_len,
           real_suffix_len);
  }

  SuRF(const std::vector<std::string>& keys, const SuffixType suffix_type,
       const level_t hash_suffix_len, const level_t real_suffix_len,
       uint32_t sparseDenseRatio) {
    create(keys, kIncludeDense, sparseDenseRatio, suffix_type, hash_suffix_len,
           real_suffix_len);
  }

  SuRF(const std::vector<std::string>& keys, const bool include_dense,
       const uint32_t sparse_dense_ratio, const SuffixType suffix_type,
       const level_t hash_suffix_len, const level_t real_suffix_len) {
    create(keys, include_dense, sparse_dense_ratio, suffix_type,
  ...
  SuRF(const std::vector<std::string>& keys,
       ...
       std::vector<bool>& right_parentheses, const bool include_dense,
       const uint32_t sparse_dense_ratio, const SuffixType suffix_type,
       const level_t hash_suffix_len, const level_t real_suffix_len,
       const uint16_t max_num_level = -1) {
    create(keys, left_parentheses, right_parentheses, include_dense,
           sparse_dense_ratio, suffix_type, hash_suffix_len, real_suffix_len,
           max_num_level);
  }
  // YCHUANG ADDED END
  ...
  void create(const std::vector<std::string>& keys, const bool include_dense,
              const uint32_t sparse_dense_ratio, const SuffixType suffix_type,
              const level_t hash_suffix_len, const level_t real_suffix_len);
  ...
  SuRF::Iter moveToKeyGreaterThan(const std::string& key,
                                  const bool inclusive) const;
  // YCHUANG ADDED START
  SuRF::Iter moveToNextCommonPrefixKey(
  ...
  uint64_t approxCount(const std::string& left_key,
  ...
  SuRF* clone() const {
    SuRF* copy = new SuRF();
    if (louds_dense_) copy->louds_dense_ = louds_dense_->clone();
    if (louds_sparse_) copy->louds_sparse_ = louds_sparse_->clone();
    return copy;
  }
  ...
  // YCHUANG ADDED START
  static SuRF* rangesToSurf(std::vector<pss> ranges,
                            size_t surf_key_length_in_bytes,
                            surf::SuffixType kSuffixType,
                            surf::level_t hash_suffix_len,
                            surf::level_t real_suffix_len, bool include_dense,
                            uint32_t sparse_dense_ratio,
                            bool flag_allow_boundary_overlapped);

  static std::pair<SuRF*, size_t> rangesWithPointKeysToSurf(
  ...
  static std::vector<std::pair<std::string, std::string>> surfToRanges(
      SuRF* surf_, bool flag_allow_boundary_overlapped);

  int getLoudsDenseHeight() const { return louds_dense_->getHeight(); }
  LoudsSparse* getLoudsSparse() const { return louds_sparse_; }
  // YCHUANG ADDED END

 private:
  LoudsDense* louds_dense_ = nullptr;
  LoudsSparse* louds_sparse_ = nullptr;
  SuRFBuilder* builder_ = nullptr;
  SuRF::Iter iter_;
  SuRF::Iter iter2_;
};
