...
class CacheTombstoneTracer {
 ...
 public:
  ...
  static std::string voidPointerToString(void* ptr) {
    ...
    std::stringstream ss;
    ss << std::hex << std::uppercase << std::setw(16) << std::setfill('0')
       << ptr_value;
  ...
  void updateCurrentlyExistKeys(
      std::unordered_set<std::string> set_currently_exist_keys) {
    std::unique_lock<std::shared_mutex> lock(rw_mutex);
    for (auto it = ych__map_tombstone_bytes.begin();
         it != ych__map_tombstone_bytes.end();) {
...
class SystemVerifier {
 private:
  ...
  unordered_map<string, vector<tuple<unsigned long long, unsigned int, bool>>>
      map_pq_tracing_info;  // key -> {(fd, LSM level, open file), ...}
  vector<tuple<unsigned long long, unsigned int, bool>>
      v_pq_tracing_info;  // {(fd, LSM level, open file), ...}
 public:
  ...
  bool usingStringKey() { return flag_using_string_key; }

  void setFlagUsingStringKey(bool flag_in) { flag_using_string_key = flag_in; }
  ...
  void setKeySize(int key_size) { this->KEY_SIZE = key_size; }
  ...
  void setFlagOpenTable() {
    ...
    std::cout << "setFlagOpenTable " << __FILE__ << ":" << __LINE__
              << std::endl;
#endif
  }
  void clearFlagOpenTable() { flag_open_table = false; }
  bool getFlagOpenTable() { return flag_open_table; }
  ...
  void clearVPQTracingInfo() { v_pq_tracing_info.clear(); }
  // void logPQTracingInfo(string &key, unsigned long long fd, unsigned int
  // LSM_level){
  void logPQTracingInfo(string key, unsigned long long fd,
                        unsigned int LSM_level) {
    if (!flag_pq_tracing_on) {
      return;
    }

    if (map_pq_tracing_info.find(key) == map_pq_tracing_info.end()) {
      map_pq_tracing_info[key] =
          vector<tuple<unsigned long long, unsigned int, bool>>();
    }

    bool flag_open_file = getFlagOpenTable();
    map_pq_tracing_info[key].push_back(
        make_tuple(fd, LSM_level, flag_open_file));
    v_pq_tracing_info.push_back(make_tuple(fd, LSM_level, flag_open_file));

    clearFlagOpenTable();
  }
  std::string getMapPQTracingInfo(std::string sep, std::string bracket,
                                  std::string prefix, int i_round) {
  ...
  std::string getVPQTracingInfo(std::string sep, std::string bracket,
                                std::string prefix, int i_round) {
    std::stringstream result;
  ...
  bool getSkipCompactionTrivialMove() { return flag_skip_trivial_move; }
  ...
  void setRunningPQ() { flag_is_running_PQ = true; }
  void resetRunningPQ() { flag_is_running_PQ = false; }

  void resetDiskAccessCount() { disk_access_count = 0; }

  void increaseDiskAccessCount() { disk_access_count++; }

  int getDiskAccessCount() { return disk_access_count; }

  void increaseeReadEntryBlockCount() { read_entry_block_count++; }

  void resetReadEntryBlockCount() { read_entry_block_count = 0; }

  int getReadEntryBlockCount() { return read_entry_block_count; }

  void increaseNumIndexReadCount() { num_index_read_count++; }
  void increaseNumFilterReadCount() { num_filter_read_count++; }
  void increaseNumRangeDelReadCount() { num_range_del_read_count++; }
  void increaseNumTotalBlockReadCount() { num_total_block_read_count++; }
  void resetNumIndexReadCount() { num_index_read_count = 0; }
  void resetNumFilterReadCount() { num_filter_read_count = 0; }
  void resetNumRangeDelReadCount() { num_range_del_read_count = 0; }
  void resetNumTotalBlockReadCount() { num_total_block_read_count = 0; }
  int getNumIndexReadCount() { return num_index_read_count; }
  int getNumFilterReadCount() { return num_filter_read_count; }
  int getNumRangeDelReadCount() { return num_range_del_read_count; }
  int getNumTotalBlockReadCount() { return num_total_block_read_count; }

  void increaseFilteredByRDFCount() { filtered_by_RDF_count++; }
  void resetFilteredByRDFCount() { filtered_by_RDF_count = 0; }
  int getFilteredByRDFCount() { return filtered_by_RDF_count; }

  void increaseBlockBasedTableOpenCount() { block_based_table_open_count++; }
  void resetBlockBasedTableOpenCount() { block_based_table_open_count = 0; }
  int getBlockBasedTableOpenCount() { return block_based_table_open_count; }

  void increaseFetcherNumCompressionDictBlockReadCount() {
    fetcher__num_compression_dict_block_read_count++;
  }
  void increaseFetcherNumIndexReadCount() { fetcher__num_index_read_count++; }
  void increaseFetcherNumFilterReadCount() { fetcher__num_filter_read_count++; }
  ...
  void increaseFetcherNumDataReadCount() { fetcher__num_data_read_count++; }
  ...
  void resetFetcherNumIndexReadCount() { fetcher__num_index_read_count = 0; }
  void resetFetcherNumFilterReadCount() { fetcher__num_filter_read_count = 0; }
  ...
  void resetFetcherNumDataReadCount() { fetcher__num_data_read_count = 0; }
  ...
  int getFetcherNumIndexReadCount() { return fetcher__num_index_read_count; }
  int getFetcherNumFilterReadCount() { return fetcher__num_filter_read_count; }
  ...
  int getFetcherNumDataReadCount() { return fetcher__num_data_read_count; }
  ...
  std::string getAllCount(std::string sep, std::string bracket,
                          std::string prefix, int N_repetitions) {
    std::stringstream result;

    result << sep << bracket << prefix << "disk_access_count" << bracket << ": "
           << std::fixed << std::setprecision(2)
           << 1.0 * disk_access_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "read_entry_block_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * read_entry_block_count / N_repetitions << "\n";
    result << "\n";

    result << sep << bracket << prefix << "duration__get_rdf" << bracket << ": "
           << std::fixed << std::setprecision(2)
           << 1.0 * get_total_duration__get_rdf() / N_repetitions << "\n";
    result << sep << bracket << prefix << "duration__get_max_seq" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * get_total_duration__get_max_seq() / N_repetitions << "\n";
    result << sep << bracket << prefix << "duration__retrieve_block" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * get_total_duration__retrieve_block() / N_repetitions
           << "\n";
    result << sep << bracket << prefix << "duration__find_table" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * get_total_duration__find_table() / N_repetitions << "\n";
    result << sep << bracket << prefix << "duration__get_from_row_cache"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * get_total_duration__get_from_row_cache() / N_repetitions
           << "\n";
    result << sep << bracket << prefix << "duration__remaining_get_path"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * get_total_duration__remaining_get_path() / N_repetitions
           << "\n";
    result << "\n";

    result << sep << bracket << prefix << "num_index_read_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * num_index_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "num_filter_read_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * num_filter_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "num_range_del_read_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * num_range_del_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "num_total_block_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * num_total_block_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "block_based_table_open_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * block_based_table_open_count / N_repetitions << "\n";

    result << "\n";

    result << sep << bracket << prefix << "filtered_by_RDF_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * filtered_by_RDF_count / N_repetitions << "\n";
    result << "\n";

    result << sep << bracket << prefix
           << "fetcher__num_compression_dict_block_read_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_compression_dict_block_read_count /
                  N_repetitions
           << "\n";
    result << sep << bracket << prefix << "fetcher__num_index_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_index_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "fetcher__num_filter_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_filter_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "fetcher__num_range_del_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_range_del_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "fetcher__num_data_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_data_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix
           << "fetcher__num_filter_partition_index_read_count" << bracket
           << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_filter_partition_index_read_count /
                  N_repetitions
           << "\n";
    result << sep << bracket << prefix << "fetcher__num_properties_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_properties_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix
           << "fetcher__num_hash_index_meta_read_count" << bracket << ": "
           << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_hash_index_meta_read_count / N_repetitions
           << "\n";
    result << sep << bracket << prefix
           << "fetcher__num_hash_index_prefixes_read_count" << bracket << ": "
           << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_hash_index_prefixes_read_count / N_repetitions
           << "\n";
    result << sep << bracket << prefix << "fetcher__num_meta_index_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_meta_index_read_count / N_repetitions << "\n";
    result << sep << bracket << prefix << "fetcher__num_total_block_read_count"
           << bracket << ": " << std::fixed << std::setprecision(2)
           << 1.0 * fetcher__num_total_block_read_count / N_repetitions << "\n";
    ...
    std::cout << prefix
              ...
              << 1.0 * fetcher__num_compression_dict_block_read_count /
                     N_repetitions
              << "\n";
    std::cout << prefix << "fetcher__num_index_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_index_read_count / N_repetitions << "\n";
    std::cout << prefix
              << "fetcher__num_filter_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_filter_read_count / N_repetitions << "\n";
    std::cout << prefix
              << "fetcher__num_range_del_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_range_del_read_count / N_repetitions
              << "\n";
    std::cout << prefix << "fetcher__num_data_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_data_read_count / N_repetitions << "\n";
    std::cout << prefix
              ...
              << "\n";
    std::cout << prefix
              << "fetcher__num_properties_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_properties_read_count / N_repetitions
              << "\n";
    std::cout << prefix
              << "fetcher__num_hash_index_meta_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_hash_index_meta_read_count / N_repetitions
              << "\n";
    std::cout << prefix << "fetcher__num_hash_index_prefixes_read_count _out = "
              << std::fixed << std::setprecision(2)
              << 1.0 * fetcher__num_hash_index_prefixes_read_count /
                     N_repetitions
              << "\n";
    std::cout << prefix
              << "fetcher__num_meta_index_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_meta_index_read_count / N_repetitions
              << "\n";
    std::cout << prefix
              << "fetcher__num_total_block_read_count _out = " << std::fixed
              << std::setprecision(2)
              << 1.0 * fetcher__num_total_block_read_count / N_repetitions
              << "\n";
  ...
  // NONE_CACHE_RANGETOMBSTONE_TRACING: is used to trace the Range Tombstone
  // lying in the cache

  // std::unordered_map<int, std::string> RDFTypes = {{0, "NONE"}, {1, "PLRDF"},
  // {2, "SPLIT_PLRDF"}}; std::unordered_map<int, std::string> RDFTypes = {{0,
  // "NONE"}, {1, "PLRDF"}}; std::unordered_map<int, std::string> RDFTypes =
  // {{0, "NONE"}, {1, "PLRDF"}, {2, "NONE"}}; std::unordered_map<int,
  // std::string> RDFTypes = {{0, "NONE"}, {1, "PLRDF"}, {2, "SPLIT_PLRDF"}, {3,
  // "NONE"}}; std::unordered_map<int, std::string> RDFTypes = {{0, "NONE"}, {1,
  // "PLRDF"}, {2, "SPLIT_PLRDF"}, {3, "TOP_LEVEL_RDF"}, {4, "NONE2"}};
  // std::unordered_map<int, std::string> RDFTypes = { {0, "NONE"}, {1,
  // "PLRDF"}, {2, "SPLIT_PLRDF"}, {3, "TOP_LEVEL_RDF"}}; // <-- debugging
  // std::unordered_map<int, std::string> RDFTypes = { {0, "SPLIT_PLRDF"}, {1,
  // "TOP_LEVEL_RDF"}}; // <-- debugging std::unordered_map<int, std::string>
  // RDFTypes = { {0, "SKYLINE_RDF"}}; // <-- debugging std::unordered_map<int,
  // std::string> RDFTypes = {{0, "NONE"}, {1, "PLRDF"}, {2, "SPLIT_PLRDF"}, {3,
  // "TOP_LEVEL_RDF"}, {4, "SKYLINE_RDF"},  {5, "NONE_DUMMY"}, {6, "NONE2"}};
  // std::unordered_map<int, std::string> RDFTypes = {{0, "NONE_DUMMY"}, {1,
  // "NONE"}, {2, "NONE2"}, {3, "PLRDF"}, {4, "SPLIT_PLRDF"}, {5,
  // "TOP_LEVEL_RDF"}, {6, "SKYLINE_RDF"},  {7, "NONE_DUMMY"}};
  // std::unordered_map<int, std::string> RDFTypes = {{0, "NONE_DUMMY"}, {1,
  // "NONE"}, {2, "NONE2"}, {3, "PLRDF"}, {4, "SPLIT_PLRDF"}, {5,
  // "TOP_LEVEL_RDF"}, {6, "SKYLINE_RDF"},  {7, "SuRF_LF_RDF"}, {8,
  // "NONE_DUMMY"}}; std::unordered_map<int, std::string> RDFTypes = {{0,
  // "NONE_DUMMY"}, {1, "NONE"}, {2, "NONE2"}, {3, "PLRDF"}, {4, "SPLIT_PLRDF"},
  // {5, "TOP_LEVEL_RDF"}, {6, "SKYLINE_RDF"},  {7, "SuRF_LF_RDF"},  {8,
  // "SuRF_LF_SPLIT_RDF"}, {9, "NONE_DUMMY"}}; std::unordered_map<int,
  // std::string> RDFTypes = {{0, "NONE_DUMMY"}, {1,
  // "NONE_CACHE_RANGETOMBSTONE_TRACING"}, {2, "NONE"}, {3, "NONE2"}, {4,
  // "PLRDF"}, {5, "SPLIT_PLRDF"}, {6, "TOP_LEVEL_RDF"}, {7, "SKYLINE_RDF"}, {8,
  // "SuRF_LF_RDF"},  {9, "SuRF_LF_SPLIT_RDF"}, {10, "NONE_DUMMY"}};
  // std::unordered_map<int, std::string> RDFTypes = {{0, "NONE_DUMMY"}, {1,
  // "NONE_CACHE_RANGETOMBSTONE_TRACING"}, {2, "NONE"}, {3, "NONE2"}, {4,
  // "PLRDF_STRING"}, {5, "SPLIT_PLRDF_STRING"}, {6, "TOP_LEVEL_RDF"}, {7,
  // "SKYLINE_RDF"},  {8, "SuRF_LF_RDF"},  {9, "SuRF_LF_SPLIT_RDF"}, {10,
  // "NONE_DUMMY"}};
  std::unordered_map<int, std::string> RDFTypes = {
      {0, "NONE_DUMMY"},
      {1, "NONE_CACHE_RANGETOMBSTONE_TRACING"},
      {2, "NONE"},
      {3, "NONE2"},
      {4, "PLRDF"},
      {5, "SPLIT_PLRDF"},
      {6, "PLRDF_STRING_KEY"},
      {7, "SPLIT_PLRDF_STRING_KEY"},
      {8, "TOP_LEVEL_RDF"},
      {9, "TOP_LEVEL_RDF_STRING_KEY"},
      {10, "SKYLINE_RDF"},
      {11, "SuRF_LF_RDF"},
      {12, "SuRF_LF_SPLIT_RDF"},
      {13, "NONE_DUMMY"}};
  // std::unordered_map<int, std::string> RDFTypes = {{0,
  // "NONE_CACHE_RANGETOMBSTONE_TRACING"}}; std::unordered_map<int, std::string>
  // RDFTypes = {{0, "SuRF_LF_RDF"}, {1, "NONE_DUMMY"}}; std::unordered_map<int,
  // std::string> RDFTypes = {{0, "SuRF_LF_SPLIT_RDF"}, {1, "NONE_DUMMY"}};
  // std::unordered_map<int, std::string> RDFTypes = {{0, "SuRF_LF_SPLIT_RDF"}};
  // std::unordered_map<int, std::string> RDFTypes = {{0, "SPLIT_PLRDF"}, {1,
  // "NONE_DUMMY"}}; std::unordered_map<int, std::string> RDFTypes = {{0,
  // "TOP_LEVEL_RDF"}};
  std::unordered_set<std::string> RDFStringKeyTypeSet = {
      "PLRDF_STRING_KEY", "SPLIT_PLRDF_STRING_KEY", "TOP_LEVEL_RDF_STRING_KEY",
      "SuRF_LF_RDF", "SuRF_LF_SPLIT_RDF"};
  ...
  void setRDFTypes(std::unordered_map<int, std::string> rdf_types_in) {
    ...
    if (usingStringKey() == true) {
      for (auto& it : rdf_types_in) {
        if (it.second.substr(0, 4) == "NONE") {
          continue;
        }
        if (RDFStringKeyTypeSet.count(it.second) != 1) {
          std::cout << "@using_string_key == 1, " << it.second
                    << " should not be used. " << __FILE__ << ":" << __LINE__
                    << " " << __FUNCTION__ << std::endl;
          std::cerr << "@using_string_key == 1, " << it.second
                    << " should not be used. " << __FILE__ << ":" << __LINE__
                    << " " << __FUNCTION__ << std::endl;
          invalid_flag = true;
        }
      ...
      for (auto& it : rdf_types_in) {
        if (it.second.substr(0, 4) == "SuRF") {
          continue;
        }
        if (RDFStringKeyTypeSet.count(it.second) == 1) {
          std::cout << "@using_string_key == 0, " << it.second
                    << " should not be used. " << __FILE__ << ":" << __LINE__
                    << " " << __FUNCTION__ << std::endl;
          std::cerr << "@using_string_key == 0, " << it.second
                    << " should not be used. " << __FILE__ << ":" << __LINE__
                    << " " << __FUNCTION__ << std::endl;
          invalid_flag = true;
        }
  ...
  uint getNumberOfRDFTypes() { return RDFTypes.size(); }

  void setRDFTypeChosed(int id) { RDFType_chosed = id; }

  std::string getStringOfRDFTypeChosed() { return RDFTypes[RDFType_chosed]; }
  ...
  void insert_deleted_keys__max_sequnce_number(string key,
                                               uint64_t max_sequnce_number) {
    deleted_keys__max_sequnce_number[key] = max_sequnce_number;
  }

  uint64_t get_deleted_keys__max_sequnce_number(string key) {
    if (deleted_keys__max_sequnce_number.find(key) ==
        deleted_keys__max_sequnce_number.end()) {
      return 0;
    }
    return deleted_keys__max_sequnce_number[key];
  }

  void set_flag_is_RDF_filtered_entry() { flag_is_RDF_filtered_entry = true; }

  void reset_flag_is_RDF_filtered_entry() {
    flag_is_RDF_filtered_entry = false;
  }

  bool get_flag_is_RDF_filtered_entry() { return flag_is_RDF_filtered_entry; }
  ...
  std::chrono::_V2::system_clock::time_point start__get_rdf =
      std::chrono::high_resolution_clock::now();
  std::chrono::_V2::system_clock::time_point stop__get_rdf =
      std::chrono::high_resolution_clock::now();
  std::chrono::nanoseconds duration__get_rdf_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(stop__get_rdf -
                                                           start__get_rdf);
  unsigned long long total_duation__get_rdf = 0;

  std::chrono::_V2::system_clock::time_point start__get_max_seq =
      std::chrono::high_resolution_clock::now();
  std::chrono::_V2::system_clock::time_point stop__get_max_seq =
      std::chrono::high_resolution_clock::now();
  std::chrono::nanoseconds duration__get_max_seq_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(stop__get_max_seq -
                                                           start__get_max_seq);
  unsigned long long total_duation__get_max_seq = 0;

  std::chrono::_V2::system_clock::time_point start__retrieve_block =
      std::chrono::high_resolution_clock::now();
  std::chrono::_V2::system_clock::time_point stop__retrieve_block =
      std::chrono::high_resolution_clock::now();
  std::chrono::nanoseconds duration__retrieve_block_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          stop__retrieve_block - start__retrieve_block);
  unsigned long long total_duation__retrieve_block = 0;

  std::chrono::_V2::system_clock::time_point start__find_table =
      std::chrono::high_resolution_clock::now();
  std::chrono::_V2::system_clock::time_point stop__find_table =
      std::chrono::high_resolution_clock::now();
  std::chrono::nanoseconds duration__find_table_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(stop__find_table -
                                                           start__find_table);
  unsigned long long total_duation__find_table = 0;

  std::chrono::_V2::system_clock::time_point start__get_from_row_cache =
      std::chrono::high_resolution_clock::now();
  std::chrono::_V2::system_clock::time_point stop__get_from_row_cache =
      std::chrono::high_resolution_clock::now();
  std::chrono::nanoseconds duration__get_from_row_cache_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
          stop__get_from_row_cache - start__get_from_row_cache);
  unsigned long long total_duation__get_from_row_cache = 0;

  std::chrono::_V2::system_clock::time_point start__remaining_get_path =
      std::chrono::high_resolution_clock::now();
  std::chrono::_V2::system_clock::time_point stop__remaining_get_path =
      std::chrono::high_resolution_clock::now();
  std::chrono::nanoseconds duration__remaining_get_path_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(
  ...
  // // vector<long long> getHistoricExistingKeys(){return
  // historicExistingKeys;}
  // // vector<long long> getCurrentlyNonInsertedKeys(){return
  // currentlyNonInsertedKeys;} void setGroundTruthSTR(map<string, string>
  // ground_truth){this->groundTruth_str = ground_truth;} void
  // setHistoricExistingKeysSTR(set<string>
  // historic_existing_keys){this->historicExistingKeys_str =
  // historic_existing_keys;} void setCurrentlyNonInsertedKeysSTR(vector<string>
  // currently_non_inserted_keys){this->currentlyNonInsertedKeys_str =
  // currently_non_inserted_keys;}
  ...
  // vector<KeyType> getCurrentlyNonInsertedKeys(){return
  // currentlyNonInsertedKeys;}
  void setGroundTruth(map<KeyType, string> ground_truth) {
    this->groundTruth = ground_truth;
  }
  void setHistoricExistingKeys(set<KeyType> historic_existing_keys) {
    this->historicExistingKeys = historic_existing_keys;
  }
  void setCurrentlyNonInsertedKeys(
      vector<KeyType> currently_non_inserted_keys) {
    this->currentlyNonInsertedKeys = currently_non_inserted_keys;
  }

  void insert(KeyType key, string value) {
    groundTruth[key] = value;
    // std::cout << "groundTruth[" << key << "] = " << groundTruth[key]  <<
    // std::endl;
  ...
  int getDeletedKeyCount() { return deleted_key_count; }
  ...
  int getCurrentlyNonInsertedKeysNum() { return CurrentlyNonInsertedKeysNum; }
  ...
  void gen_workload_with_numbers_of_PQ(
      int N_repetitions, int number_of_PQs_on_existing_keys,
      int number_of_PQs_on_historic_existing_keys,
      int number_of_PQs_on_currently_deleted_keys,
      int number_of_PQs_on_currently_non_inserted_keys, int key_length = 12) {
    ...
    // int number_of_PQs_on_existing_keys, int
    // number_of_PQs_on_historic_existing_keys, int
    // number_of_PQs_on_currently_deleted_keys, int
    // number_of_PQs_on_currently_non_inserted_keys
    ...
    if (number_of_PQs_on_existing_keys == -1) {
      ...
      workload_all_existing_keys = vector<vector<KeyType>>(
          N_repetitions, vector<KeyType>(number_of_PQs_on_existing_keys));
    ...
    if (number_of_PQs_on_historic_existing_keys == -1) {
      ...
      workload_historic_existing_keys = vector<vector<KeyType>>(
          N_repetitions,
          vector<KeyType>(number_of_PQs_on_historic_existing_keys));
    ...
    if (number_of_PQs_on_currently_deleted_keys == -1) {
      ...
      if (len_currently_deleted_keys <= 0) {
        workload_currently_deleted_keys =
            vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
      } else {
        workload_currently_deleted_keys = vector<vector<KeyType>>(
            N_repetitions,
            vector<KeyType>(number_of_PQs_on_currently_deleted_keys));
    ...
    if (number_of_PQs_on_currently_non_inserted_keys == -1) {
      for (int i = 0; i < N_repetitions; i++) {
        workload_currently_non_inserted_keys.push_back(
            currently_non_inserted_keys);
      }
    } else {
      workload_currently_non_inserted_keys = vector<vector<KeyType>>(
          N_repetitions,
          vector<KeyType>(number_of_PQs_on_currently_non_inserted_keys));
      ...
      for (int i = 0; i < N_repetitions; i++) {
        ...
        for (int j = 0; j < number_of_PQs_on_currently_non_inserted_keys; j++) {
          ...
          workload_currently_non_inserted_keys[i][j] =
              currently_non_inserted_keys[idx];
        }
      }
    }
  ...
  // void load_workload_with_numbers_of_PQ(int N_repetitions, int number_of_PQs,
  // const std::string workload_file_name){
  void load_workload_with_numbers_of_PQ(
      int N_repetitions, int number_of_PQs_on_existing_keys,
      int number_of_PQs_on_historic_existing_keys,
      int number_of_PQs_on_currently_deleted_keys,
      ...
      const std::string workload_file_name) {
    {
      // const string pq_workload_ground_truth_file_name =
      // "../../development/self_RD/" + workload_file_name + "_ground_truth";
      // const string pq_workload_historic_existing_file_name =
      // "../../development/self_RD/" +  workload_file_name +
      // "_historic_existing"; const string
      // pq_workload_currently_non_existed_file_name =
      // "../../development/self_RD/" + workload_file_name +
      // "_currently_non_existed";

      const string pq_workload_ground_truth_file_name =
          workload_file_name + "_ground_truth";
      const string pq_workload_historic_existing_file_name =
          workload_file_name + "_historic_existing";
      const string pq_workload_currently_non_existed_file_name =
          workload_file_name + "_currently_non_existed";

      // Read the map back from the file
      std::cout << "load " << pq_workload_ground_truth_file_name << std::endl;
      std::map<KeyType, std::string> read_back_map =
          readDictFromFile<KeyType, std::string>(
              pq_workload_ground_truth_file_name);
      this->setGroundTruth(read_back_map);

      // Read the vector back from the file
      std::cout << "load " << pq_workload_historic_existing_file_name
                << std::endl;
      std::set<KeyType> read_back_vector =
          readSetFromFile<KeyType>(pq_workload_historic_existing_file_name);
      this->setHistoricExistingKeys(read_back_vector);

      // Read the vector back from the file
      std::cout << "load " << pq_workload_currently_non_existed_file_name
                << std::endl;
      std::set<KeyType> read_back_vector2 =
          readSetFromFile<KeyType>(pq_workload_currently_non_existed_file_name);
    ...
    if (number_of_PQs_on_existing_keys == -1) {
      workload_all_existing_keys =
          vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
      {
        const string pq_workload_all_existing_keys_file_name =
            workload_file_name + "_all_existing_keys";

        for (int i = 0; i < N_repetitions; i++) {
          // Read the vector back from the file
          std::cout << "load " << pq_workload_all_existing_keys_file_name
                    << std::endl;
          auto read_back_vector_all_existing_keys = readVectorFromFile<KeyType>(
              pq_workload_all_existing_keys_file_name);
      ...
      for (int i = 0; i < N_repetitions; i++) {
        const string pq_workload_all_existing_keys_file_name =
            workload_file_name + "_all_existing_keys" + "_round_" +
            to_string(i) + "_number_of_pq_" +
            to_string(number_of_PQs_on_existing_keys);
    ...
    if (number_of_PQs_on_historic_existing_keys == -1) {
      workload_historic_existing_keys =
          vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
      {
        const string pq_workload_historic_existing_keys_file_name =
            workload_file_name + "_historic_existing_keys";

        for (int i = 0; i < N_repetitions; i++) {
          // Read the vector back from the file
          std::cout << "load " << pq_workload_historic_existing_keys_file_name
                    << std::endl;
          auto read_back_vector_historic_existing_keys =
              readVectorFromFile<KeyType>(
                  pq_workload_historic_existing_keys_file_name);

          workload_historic_existing_keys[i] =
              (read_back_vector_historic_existing_keys);
        }
      }
    } else {
      workload_historic_existing_keys = vector<vector<KeyType>>(
      ...
      for (int i = 0; i < N_repetitions; i++) {
        const string pq_workload_historic_existing_keys_file_name =
            workload_file_name + "_historic_existing_keys" + "_round_" +
            to_string(i) + "_number_of_pq_" +
            to_string(number_of_PQs_on_historic_existing_keys);
    ...
    if (number_of_PQs_on_currently_deleted_keys == -1) {
      workload_currently_deleted_keys =
          vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
      {
        const string pq_workload_currently_deleted_keys_file_name =
            workload_file_name + "_currently_deleted_keys";

        for (int i = 0; i < N_repetitions; i++) {
          // Read the vector back from the file
          std::cout << "load " << pq_workload_currently_deleted_keys_file_name
                    << std::endl;
          auto read_back_vector_currently_deleted_keys =
              readVectorFromFile<KeyType>(
                  pq_workload_currently_deleted_keys_file_name);

          workload_currently_deleted_keys[i] =
              (read_back_vector_currently_deleted_keys);
        }
      }
    } else {
      workload_currently_deleted_keys = vector<vector<KeyType>>(
      ...
      for (int i = 0; i < N_repetitions; i++) {
        const string pq_workload_currently_deleted_keys_file_name =
            workload_file_name + "_currently_deleted_keys" + "_round_" +
            to_string(i) + "_number_of_pq_" +
            to_string(number_of_PQs_on_currently_deleted_keys);
    ...
    if (number_of_PQs_on_currently_non_inserted_keys == -1) {
      workload_currently_non_inserted_keys =
          vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
      {
        const string pq_workload_currently_non_inserted_keys_file_name =
            workload_file_name + "_currently_non_inserted_keys";

        for (int i = 0; i < N_repetitions; i++) {
          // Read the vector back from the file
          std::cout << "load "
                    << pq_workload_currently_non_inserted_keys_file_name
                    << std::endl;
          auto read_back_vector_currently_non_inserted_keys =
              readVectorFromFile<KeyType>(
                  pq_workload_currently_non_inserted_keys_file_name);

          workload_currently_non_inserted_keys[i] =
              (read_back_vector_currently_non_inserted_keys);
        }
      }
    } else {
      workload_currently_non_inserted_keys = vector<vector<KeyType>>(
      ...
      for (int i = 0; i < N_repetitions; i++) {
        const string pq_workload_currently_non_inserted_keys_file_name =
            workload_file_name + "_currently_non_inserted_keys" + "_round_" +
            to_string(i) + "_number_of_pq_" +
            to_string(number_of_PQs_on_currently_non_inserted_keys);
    ...
    //   workload_all_existing_keys = vector<vector<KeyType>>(N_repetitions,
    //   vector<KeyType>()); workload_historic_existing_keys =
    //   vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
    //   workload_currently_deleted_keys =
    //   vector<vector<KeyType>>(N_repetitions, vector<KeyType>());
    //   workload_currently_non_inserted_keys =
    ...
    //       // const string pq_workload_historic_existing_keys_file_name =
    //       "../../development/self_RD/" + workload_file_name +
    //       "_historic_existing_keys";
    //       // const string pq_workload_currently_deleted_keys_file_name =
    //       "../../development/self_RD/" + workload_file_name +
    //       "_currently_deleted_keys";
    //       // const string pq_workload_currently_non_inserted_keys_file_name =
    //       "../../development/self_RD/" + workload_file_name +
    //       "_currently_non_inserted_keys";

    //       const string pq_workload_all_existing_keys_file_name =
    //       workload_file_name + "_all_existing_keys"; const string
    //       pq_workload_historic_existing_keys_file_name = workload_file_name +
    //       "_historic_existing_keys"; const string
    //       pq_workload_currently_deleted_keys_file_name = workload_file_name +
    //       "_currently_deleted_keys"; const string
    //       pq_workload_currently_non_inserted_keys_file_name =
    //       workload_file_name + "_currently_non_inserted_keys";

    //       // vector<KeyType> workload_all_existing_keys =
    //       system_verifier->getAllExistingKeys();
    //       // vector<KeyType> workload_historic_existing_keys =
    //       system_verifier->getHistoricExistingKeys();
    //       // vector<KeyType> workload_currently_deleted_keys =
    //       system_verifier->getCurrentlyDeletedKeys();
    //       // vector<KeyType> workload_currently_non_inserted_keys =
    //       system_verifier->getCurrentlyNonInsertedKeys();
    ...
    //         // Read the vector back from the file
    //         std::cout << "load " << pq_workload_all_existing_keys_file_name
    //         << std::endl; auto read_back_vector_all_existing_keys =
    //         readVectorFromFile<KeyType>(pq_workload_all_existing_keys_file_name);

    //         // Read the vector back from the file
    //         std::cout << "load " <<
    //         pq_workload_historic_existing_keys_file_name << std::endl; auto
    //         read_back_vector_historic_existing_keys =
    //         readVectorFromFile<KeyType>(pq_workload_historic_existing_keys_file_name);

    //         // Read the vector back from the file
    //         std::cout << "load " <<
    //         pq_workload_currently_deleted_keys_file_name << std::endl; auto
    //         read_back_vector_currently_deleted_keys =
    //         readVectorFromFile<KeyType>(pq_workload_currently_deleted_keys_file_name);

    //         // Read the vector back from the file
    //         std::cout << "load " <<
    //         pq_workload_currently_non_inserted_keys_file_name << std::endl;
    //         auto read_back_vector_currently_non_inserted_keys =
    //         readVectorFromFile<KeyType>(pq_workload_currently_non_inserted_keys_file_name);

    //         workload_all_existing_keys[i] =
    //         (read_back_vector_all_existing_keys);
    //         workload_historic_existing_keys[i] =
    //         (read_back_vector_historic_existing_keys);
    //         workload_currently_deleted_keys[i] =
    //         (read_back_vector_currently_deleted_keys);
    ...
    // }else{
    //   workload_all_existing_keys = vector<vector<KeyType>>(N_repetitions,
    //   vector<KeyType>(number_of_PQs)); workload_historic_existing_keys =
    //   vector<vector<KeyType>>(N_repetitions, vector<KeyType>(number_of_PQs));
    //   workload_currently_deleted_keys =
    //   vector<vector<KeyType>>(N_repetitions, vector<KeyType>(number_of_PQs));
    //   workload_currently_non_inserted_keys =
    //   vector<vector<KeyType>>(N_repetitions, vector<KeyType>(number_of_PQs));
    ...
    //     // const string pq_workload_all_existing_keys_file_name =
    //     "../../development/self_RD/" + workload_file_name +
    //     "_all_existing_keys" + "_round_" + to_string(i) + "_number_of_pq_" +
    //     to_string(number_of_PQs);
    //     // const string pq_workload_historic_existing_keys_file_name =
    //     "../../development/self_RD/" + workload_file_name +
    //     "_historic_existing_keys" + "_round_" + to_string(i) +
    //     "_number_of_pq_" + to_string(number_of_PQs);
    //     // const string pq_workload_currently_deleted_keys_file_name =
    //     "../../development/self_RD/" + workload_file_name +
    //     "_currently_deleted_keys" + "_round_" + to_string(i) +
    //     "_number_of_pq_" + to_string(number_of_PQs);
    //     // const string pq_workload_currently_non_inserted_keys_file_name =
    //     "../../development/self_RD/" + workload_file_name +
    //     "_currently_non_inserted_keys" + "_round_" + to_string(i) +
    //     "_number_of_pq_" + to_string(number_of_PQs);

    //     const string pq_workload_all_existing_keys_file_name =
    //     workload_file_name + "_all_existing_keys" + "_round_" + to_string(i)
    //     + "_number_of_pq_" + to_string(number_of_PQs); const string
    //     pq_workload_historic_existing_keys_file_name = workload_file_name +
    //     "_historic_existing_keys" + "_round_" + to_string(i) +
    //     "_number_of_pq_" + to_string(number_of_PQs); const string
    //     pq_workload_currently_deleted_keys_file_name = workload_file_name +
    ...
    //     pq_workload_currently_non_inserted_keys_file_name =
    //     workload_file_name + "_currently_non_inserted_keys" + "_round_" +
    //     to_string(i) + "_number_of_pq_" + to_string(number_of_PQs);
    ...
    //     // Read the vector back from the file
    //     std::cout << "load " << pq_workload_historic_existing_keys_file_name
    //     << std::endl; auto read_back_vector_historic_existing_keys =
    //     readVectorFromFile<KeyType>(pq_workload_historic_existing_keys_file_name);

    //     // Read the vector back from the file
    //     std::cout << "load " << pq_workload_currently_deleted_keys_file_name
    //     << std::endl; auto read_back_vector_currently_deleted_keys =
    //     readVectorFromFile<KeyType>(pq_workload_currently_deleted_keys_file_name);

    //     // Read the vector back from the file
    //     std::cout << "load " <<
    //     pq_workload_currently_non_inserted_keys_file_name << std::endl; auto
    //     read_back_vector_currently_non_inserted_keys =
    //     readVectorFromFile<KeyType>(pq_workload_currently_non_inserted_keys_file_name);
  ...
  vector<KeyType> getHistoricExistingKeys() {
    vector<KeyType> result;
    for (auto it = historicExistingKeys.begin();
         it != historicExistingKeys.end(); it++) {
      result.push_back(*it);
    }
    return result;
  }
  ...
  void genCurrentlyNonInsertedKeys(int num, int length = 12) {
    ...
    while (num) {
      ...
      KeyType key =
          getRandomString(length);  // we need to ensure that the key extends to
                                    // the whole char_set/digit_set space
      if (groundTruth.count(key) == 0) {
        currentlyNonInsertedKeys.push_back(key);
        num--;
      }
  ...
  std::string getCurrentlyDeletedKeysVec2dString(std::string sep,
                                                 std::string bracket,
                                                 std::string prefix) {
    std::stringstream result;
