...
class verification_runner {
 private:
  ...
  static PLRDF_t<std::string> plrdf_stringkey_prime,
      split_plrdf_stringkey_prime;
 ...
 public:
  static void initPQVerification(DB** db_ptr2, ReadOptions& read_op,
                                 EmuEnv* _env);
  // static void runPQVerification(DB** db_ptr2, Options& op, WriteOptions&
  // write_op,
  //                         ReadOptions& read_op, EmuEnv* _env, int
  //                         number_of_PQs = -1, string kDBPath =
  //                         "/tmp/cs561_project1");
  static void runPQVerification(
      DB** db_ptr2, Options& op, WriteOptions& write_op, ReadOptions& read_op,
      EmuEnv* _env,
      // int number_of_PQs = -1,
...
PLRDF_t<std::string> verification_runner::plrdf_stringkey_prime,
    verification_runner::split_plrdf_stringkey_prime;
...
void verification_runner::initPQVerification(DB** db_ptr2, ReadOptions& read_op,
                                             EmuEnv* _env) {
  DB* db = *db_ptr2;
  Status s;
  ...
  for (auto x : system_verifier->getCurrentlyDeletedKeys()) {
    ...
    if (flag_using_string_key == 0) {
      ...
      // __FUNCTION__ << std::endl; key = ss_key.str();
    } else {
      searching_key << x;
    }
  ...
  std::string testing_result_file_name =
      _env->logging_filename + ".testing_log";
  std::string testing_result_file_name2 =
      "output_statistics/" + _env->logging_filename + ".json";
  std::cout << "testing_result_file_name2 =  " << testing_result_file_name2
  ...
  testing_result_file << "PLRDF Number Of Total Ranges: "
                      << db->getPLRDFNumberOfTotalRanges() << std::endl;
  testing_result_file << "Split PLRDF Number Of Total Ranges: "
                      << db->getSplitPLRDFNumberOfTotalRanges() << std::endl;
  testing_result_file << "PLRDF StringKey Number Of Total Ranges: "
                      << db->getPLRDFStringKeyNumberOfTotalRanges()
                      << std::endl;
  testing_result_file << "Split PLRDF StringKey Number Of Total Ranges: "
                      << db->getSplitPLRDFStringKeyNumberOfTotalRanges()
                      << std::endl;
  testing_result_file << "TopLevel RDF Number Of Total Ranges: "
                      << db->getTopLevelRDFNumberOfTotalRanges() << std::endl;
  testing_result_file << "TopLevel RDF StringKey Number Of Total Ranges: "
                      << db->getTopLevelRDFStringKeyNumberOfTotalRanges()
                      << std::endl;
  testing_result_file << "Skyline RDF Number Of Total Ranges: "
                      << db->getSkylineRDFNumberOfTotalRanges() << std::endl;
  testing_result_file << "SuRF Level File RDF Number Of Total Ranges: "
                      << db->getSuRFLevelFileRDFNumberOfTotalRanges()
                      << std::endl;
  testing_result_file << "SuRF Level File Split RDF Number Of Total Ranges: "
                      << db->getSuRFLevelFileSplitRDFNumberOfTotalRanges()
                      << std::endl;
  testing_result_file << std::endl;

  testing_result_file2 << ",\"PLRDF Number Of Total Ranges\" : "
                       << db->getPLRDFNumberOfTotalRanges() << std::endl;
  testing_result_file2 << ",\"Split PLRDF Number Of Total Ranges\" : "
                       << db->getSplitPLRDFNumberOfTotalRanges() << std::endl;
  testing_result_file2 << ",\"PLRDF StringKey Number Of Total Ranges\" : "
                       << db->getPLRDFStringKeyNumberOfTotalRanges()
                       << std::endl;
  testing_result_file2 << ",\"Split PLRDF StringKey Number Of Total Ranges\" : "
                       << db->getSplitPLRDFStringKeyNumberOfTotalRanges()
                       << std::endl;
  testing_result_file2 << ",\"TopLevel RDF Number Of Total Ranges\" : "
                       << db->getTopLevelRDFNumberOfTotalRanges() << std::endl;
  testing_result_file2
      << ",\"TopLevel RDF StringKey Number Of Total Ranges\" : "
      << db->getTopLevelRDFStringKeyNumberOfTotalRanges() << std::endl;
  testing_result_file2 << ",\"Skyline RDF Number Of Total Ranges\" : "
                       << db->getSkylineRDFNumberOfTotalRanges() << std::endl;
  testing_result_file2 << ",\"SuRF Level File RDF Number Of Total Ranges\" : "
                       << db->getSuRFLevelFileRDFNumberOfTotalRanges()
                       << std::endl;
  testing_result_file2
      << ",\"SuRF Level File Split RDF Number Of Total Ranges\" : "
      << db->getSuRFLevelFileSplitRDFNumberOfTotalRanges() << std::endl;
  testing_result_file2 << std::endl;

  testing_result_file << "RocksDB (None) Number Of Total Memory Usage: "
                      << db->getRTRocksDBNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "PLRDF Number Of Total Memory Usage: "
                      << db->getPLRDFNumberOfTotalMemoryUsage() << std::endl;
  testing_result_file << "Split PLRDF Number Of Total Memory Usage: "
                      << db->getSplitPLRDFNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "PLRDF StringKey Number Of Total Memory Usage: "
                      << db->getPLRDFStringKeyNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "Split PLRDF StringKey Number Of Total Memory Usage: "
                      << db->getSplitPLRDFStringKeyNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "TopLevel RDF Number Of Total Memory Usage: "
                      << db->getTopLevelRDFNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "TopLevel RDF StringKey Number Of Total Memory Usage: "
                      << db->getTopLevelRDFStringKeyNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "Skyline RDF Number Of Total Memory Usage: "
                      << db->getSkylineRDFNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file << "SuRF Level File RDF Number Of Total Memory Usage: "
                      << db->getSuRFLevelFileRDFNumberOfTotalMemoryUsage()
                      << std::endl;
  testing_result_file
      << "SuRF Level File Split RDF Number Of Total Memory Usage: "
      << db->getSuRFLevelFileSplitRDFNumberOfTotalMemoryUsage() << std::endl;
  testing_result_file << std::endl;

  testing_result_file2 << ",\"RocksDB (None) Number Of Total Memory Usage\" : "
                       << db->getRTRocksDBNumberOfTotalMemoryUsage()
                       << std::endl;
  // testing_result_file2 << ",\"RocksDB (None) Number Of Total Memory Usage
  // Included Timestamp\" : " <<
  // db->getRTRocksDBNumberOfTotalMemoryUsageIncludedTimestamp() << std::endl;
  testing_result_file2 << ",\"PLRDF Number Of Total Memory Usage\" : "
                       << db->getPLRDFNumberOfTotalMemoryUsage() << std::endl;
  testing_result_file2 << ",\"Split PLRDF Number Of Total Memory Usage\" : "
                       << db->getSplitPLRDFNumberOfTotalMemoryUsage()
                       << std::endl;
  testing_result_file2 << ",\"PLRDF StringKey Number Of Total Memory Usage\" : "
                       << db->getPLRDFStringKeyNumberOfTotalMemoryUsage()
                       << std::endl;
  testing_result_file2
      << ",\"Split PLRDF StringKey Number Of Total Memory Usage\" : "
      << db->getSplitPLRDFStringKeyNumberOfTotalMemoryUsage() << std::endl;
  testing_result_file2 << ",\"TopLevel RDF Number Of Total Memory Usage\" : "
                       << db->getTopLevelRDFNumberOfTotalMemoryUsage()
                       << std::endl;
  testing_result_file2
      << ",\"TopLevel RDF StringKey Number Of Total Memory Usage\" : "
      << db->getTopLevelRDFStringKeyNumberOfTotalMemoryUsage() << std::endl;
  testing_result_file2 << ",\"Skyline RDF Number Of Total Memory Usage\" : "
                       << db->getSkylineRDFNumberOfTotalMemoryUsage()
                       << std::endl;
  testing_result_file2
      << ",\"SuRF Level File RDF Number Of Total Memory Usage\" : "
  ...
  // std::cout << "RocksDB (None) Number Of Total Memory Usage Origin Included
  // Timestamp _out = " <<
  // db->getRTRocksDBNumberOfTotalMemoryUsageIncludedTimestamp() << std::endl;
  ...
  testing_result_file2 << "]" << std::endl;
  testing_result_file2
      << ",\"Log Of Numbers Of Ranges In SplitPLRDFStringKey\" : [";
  ...
  testing_result_file2 << "]" << std::endl;
  testing_result_file2
      << ",\"Log Of Numbers Of Ranges In TopLevelRDFStringKey\" : [";
  ...
  testing_result_file2 << "]" << std::endl;
  testing_result_file2
      << ",\"Log Of Numbers Of Ranges In SuRFLevelFileRDF\" : [";
  ...
  testing_result_file2 << "]" << std::endl;
  testing_result_file2
      << ",\"Log Of Numbers Of Ranges In SuRFLevelFileSplitRDF\" : [";
  ...
  ;
  testing_result_file2
      << ",\"Log Of Memory Usage Of TopLevelRDFStringKey\" : [";
  ...
  testing_result_file2 << "]" << std::endl;
  testing_result_file2
      << ",\"Log Of Memory Usage Of SuRFLevelFileSplitRDF\" : [";
  ...
  const long long N_repetitions =
      checking::SystemVerifier::EXPERIMENT_REPETITION_TIMES;
  testing_result_file << "N_repetitions = " << N_repetitions << std::endl
                      << std::endl;
  testing_result_file2 << ",\"N_repetitions\" : " << N_repetitions << std::endl;
  testing_result_file2 << ",\"Total number of SST Files\" : "
                       << db->getTotalNumberOfSSTFiles() << std::endl
                       << std::endl;
...
void verification_runner::runPQVerification(
    DB** db_ptr2, Options& op, WriteOptions& write_op, ReadOptions& read_op,
    EmuEnv* _env,
    // int number_of_PQs,
    ...
    int number_of_PQs_on_currently_non_inserted_keys, string kDBPath) {
  // std::cout << "number_of_PQ = " << number_of_PQs <<  std::endl;
  std::cout << "number_of_PQs_on_existing_keys = "
            << number_of_PQs_on_existing_keys << std::endl;
  std::cout << "number_of_PQs_on_historic_existing_keys = "
            << number_of_PQs_on_historic_existing_keys << std::endl;
  std::cout << "number_of_PQs_on_currently_deleted_keys = "
            << number_of_PQs_on_currently_deleted_keys << std::endl;
  std::cout << "number_of_PQs_on_currently_non_inserted_keys = "
            << number_of_PQs_on_currently_non_inserted_keys << std::endl;
  ...
  //   prefix_number_of_PQs = "fixed #PQ = " +
  //   std::to_string(number_of_PQs_on_existing_keys);
  // }

  std::string prefix_number_of_PQs_on_existing_keys = "";
  if (number_of_PQs_on_existing_keys != -1) {
    prefix_number_of_PQs_on_existing_keys =
        "fixed #PQ = " + std::to_string(number_of_PQs_on_existing_keys);
  }

  std::string prefix_number_of_PQs_on_historic_existing_keys = "";
  if (number_of_PQs_on_historic_existing_keys != -1) {
    prefix_number_of_PQs_on_historic_existing_keys =
        "fixed #PQ = " +
        std::to_string(number_of_PQs_on_historic_existing_keys);
  }

  std::string prefix_number_of_PQs_on_currently_deleted_keys = "";
  if (number_of_PQs_on_currently_deleted_keys != -1) {
    prefix_number_of_PQs_on_currently_deleted_keys =
        "fixed #PQ = " +
        std::to_string(number_of_PQs_on_currently_deleted_keys);
  }

  std::string prefix_number_of_PQs_on_currently_non_inserted_keys = "";
  if (number_of_PQs_on_currently_non_inserted_keys != -1) {
    prefix_number_of_PQs_on_currently_non_inserted_keys =
        "fixed #PQ = " +
        std::to_string(number_of_PQs_on_currently_non_inserted_keys);
  }

  // prefix_number_of_PQs_on_existing_keys,
  // prefix_number_of_PQs_on_historic_existing_keys,
  // prefix_number_of_PQs_on_currently_deleted_keys,
  // prefix_number_of_PQs_on_currently_non_inserted_keys std::cout <<
  // "number_of_PQs_on_existing_keys = " << number_of_PQs_on_existing_keys <<
  // std::endl; std::cout << "number_of_PQs_on_historic_existing_keys = " <<
  // number_of_PQs_on_historic_existing_keys <<  std::endl; std::cout <<
  // "number_of_PQs_on_currently_deleted_keys = " <<
  // number_of_PQs_on_currently_deleted_keys <<  std::endl; std::cout <<
  ...
  if (_env->load_pq_workload == false) {
    ...
    // number_of_PQs);
    system_verifier->gen_workload_with_numbers_of_PQ(
        N_repetitions, number_of_PQs_on_existing_keys,
        number_of_PQs_on_historic_existing_keys,
        number_of_PQs_on_currently_deleted_keys,
        number_of_PQs_on_currently_non_inserted_keys);
  } else {
    // system_verifier->load_workload_with_numbers_of_PQ(N_repetitions,
    // number_of_PQs, _env->workload_file_name);
    system_verifier->load_workload_with_numbers_of_PQ(
        N_repetitions, number_of_PQs_on_existing_keys,
        number_of_PQs_on_historic_existing_keys,
        number_of_PQs_on_currently_deleted_keys,
        number_of_PQs_on_currently_non_inserted_keys, _env->workload_file_name);
  }
  // testing_result_file2 <<
  // system_verifier->getCurrentlyDeletedKeysVec2dString(",", "\"",
  // prefix_number_of_PQs) << std::endl;
  testing_result_file2 << system_verifier->getCurrentlyDeletedKeysVec2dString(
                              ",", "\"",
                              prefix_number_of_PQs_on_currently_deleted_keys)
                       << std::endl;
  ...
  auto duration_pq =
      std::chrono::duration_cast<std::chrono::nanoseconds>(stop_pq - start_pq);
  ...
  for (uint t = 0; t < system_verifier->getNumberOfRDFTypes(); t++) {
    ...
    clear_RDF_false_positive_rate_generic(db, system_verifier);

    if (system_verifier->getStringOfRDFTypeChosed() == "NONE_DUMMY") {
      s = db->SetOptions(
          {{"max_open_files",
            "1"}});  // is there any compaction happended after this????
    } else {
      s = db->SetOptions(
          {{"max_open_files",
            std::to_string(
                _env->max_open_files)}});  // is there any compaction happended
                                           // after this????
    }

    // testing_result_file << system_verifier->getStringOfRDFTypeChosed() << " "
    // << prefix_number_of_PQs << std::endl;
    testing_result_file << system_verifier->getStringOfRDFTypeChosed() << " "
                        << prefix_number_of_PQs_on_existing_keys << std::endl;
    ...
    for (auto i = 0; i < N_repetitions; i++) {
      ...
      if (flag_reopen_db_for_each_RDF_testing == true) {
        retrieve_all_RDFs(db_ptr2, plrdf_prime, split_plrdf_prime,
                          plrdf_stringkey_prime, split_plrdf_stringkey_prime,
                          top_level_rdf_prime, top_level_rdf_stringkey_prime,
                          skyline_rdf_prime);
        reopen_DB(db_ptr2, op, write_op, read_op, _env, kDBPath);
        set_all_RDFs(db_ptr2, plrdf_prime, split_plrdf_prime,
                     plrdf_stringkey_prime, split_plrdf_stringkey_prime,
                     top_level_rdf_prime, top_level_rdf_stringkey_prime,
                     skyline_rdf_prime);
      }
      ...
      for (auto x : system_verifier->getAllExistingKeysAtNRound(i)) {
        ...
        duration_pq = std::chrono::duration_cast<std::chrono::nanoseconds>(
            stop_pq - start_pq);
        ...
        if (system_verifier->getStringOfRDFTypeChosed() ==
            "NONE_CACHE_RANGETOMBSTONE_TRACING") {
          uint64_t bytes = checking::CacheTombstoneTracer::getInstance()
                               ->getTotalTombstoneBytes();
        ...
        if (s.ok() != gt_is_exist) {
          ...
          std::cout << "ERROR (Existence inconsistency): " << x
                    ...
                    << std::endl;
#endif
          testing_result_file << "ERROR (Existence inconsistency): " << x
                              << " (result, gt_result) " << s.ok() << " "
                              << gt_is_exist << std::endl;
        }
        ...
        if (value != gt_value) {
          ...
          std::cout << "ERROR (Value inconsistency): " << x
                    ...
                    << std::endl;
#endif
          testing_result_file << "ERROR (Value inconsistency): " << x
                              << " (value, gt_value) " << value << " "
                              << gt_value << std::endl;
        }
      }
      if (system_verifier->getStringOfRDFTypeChosed() == "NONE_DUMMY") {
      ...
      if (system_verifier->getStringOfRDFTypeChosed() ==
        ...
        // std::string prefix = " (Existing Keys " + i + prefix_number_of_PQs +
        // " " + i_round_str + ") " +
        // system_verifier->getStringOfRDFTypeChosed() + " ";
        std::string prefix = " (Existing Keys " + i +
                             prefix_number_of_PQs_on_existing_keys + " " +
                             i_round_str + ") " +
                             system_verifier->getStringOfRDFTypeChosed() + " ";
      ...
      testing_result_file << " Disk Access count = "
                          << system_verifier->getDiskAccessCount() << std::endl;
      testing_logger.set_to_end(op, testing_result_file);
    }
    // std::cout << " rocksdb::get_perf_context()->bloom_sst_miss_count = " <<
    // rocksdb::get_perf_context()->bloom_sst_miss_count << " " << __FILE__ <<
    // ":" << __LINE__ << " " << __FUNCTION__ << std::endl; std::cout << "
    // rocksdb::get_perf_context()->bloom_sst_hit_count = " <<
    // rocksdb::get_perf_context()->bloom_sst_hit_count << " " << __FILE__ <<
    // ":" << __LINE__ << " " << __FUNCTION__ << std::endl;

    if (system_verifier->getStringOfRDFTypeChosed() == "NONE_DUMMY") {
      continue;
    }
    double block_read_cpu_time =
        parsing_value_from_string(rocksdb::get_perf_context()->ToString(),
                                  ".*block_read_cpu_time = ([0-9.]+)");

    testing_result_file << system_verifier->getStringOfRDFTypeChosed() << " "
                        << std::fixed << std::setprecision(2)
                        << "Average Disk Access count = "
                        << 1.0 * disk_access_count / N_repetitions << std::endl;
    testing_result_file << system_verifier->getStringOfRDFTypeChosed() << " "
                        << std::fixed << std::setprecision(2)
                        << " elapsed time = "
                        << 1.0 * point_query_time / N_repetitions / 1e6
                        << " (ms) " << std::endl
                        << std::endl;
    testing_result_file << "filtered by RDF count = " << std::fixed
                        << std::setprecision(2)
                        << 1.0 * system_verifier->getFilteredByRDFCount() /
                               N_repetitions
                        << std::endl;
    // if (number_of_PQs == -1){
    //   testing_result_file << "number of PQ = " <<
    //   system_verifier->getAllExistingKeys().size() << std::endl;
    // }else{
    //   testing_result_file << "number of PQ = " << number_of_PQs << std::endl;
    // }
    if (number_of_PQs_on_existing_keys == -1) {
      testing_result_file << "number of PQ = "
                          << system_verifier->getAllExistingKeys().size()
                          << std::endl;
    } else {
      testing_result_file << "number of PQ = " << number_of_PQs_on_existing_keys
                          << std::endl;
    }
    testing_result_file << system_verifier->getAllCount("", "", "",
                                                        N_repetitions)
                        << std::endl;
    testing_result_file << "block_read_cpu_time = "
                        << 1.0 * block_read_cpu_time / N_repetitions / 1e3 << ""
                        << std::endl;

    // std::string prefix = " (Exist Keys) " +
    // system_verifier->getStringOfRDFTypeChosed() + " "; std::string prefix = "
    // (Exist Keys " + prefix_number_of_PQs + ") "+
    // system_verifier->getStringOfRDFTypeChosed() + " ";
    std::string prefix = " (Exist Keys " +
                         prefix_number_of_PQs_on_existing_keys + ") " +
                         system_verifier->getStringOfRDFTypeChosed() + " ";
    testing_result_file2 << ",\"" + prefix + " elapsed time\" : "
                         << 1.0 * point_query_time / N_repetitions / 1e6
                         << std::endl;
    testing_result_file2 << ",\"" + prefix + " filtered by RDF count\" : "
                         << std::fixed << std::setprecision(2)
                         << 1.0 * system_verifier->getFilteredByRDFCount() /
                                N_repetitions
                         << std::endl;
    // if (number_of_PQs == -1){
    //   testing_result_file2 << ",\"" + prefix + " number of PQ\" : " <<
    //   system_verifier->getAllExistingKeys().size() << std::endl;
    // }else{
    //   testing_result_file2 << ",\"" + prefix + " number of PQ\" : " <<
    //   number_of_PQs << std::endl;
    // }
    if (number_of_PQs_on_existing_keys == -1) {
      testing_result_file2 << ",\"" + prefix + " number of PQ\" : "
                           << system_verifier->getAllExistingKeys().size()
    ...
    testing_result_file2 << system_verifier->getAllCount(",", "\"", prefix,
                                                         N_repetitions)
                         << std::endl;
    testing_result_file2 << ",\"" + prefix + " block_read_cpu_time\" : "
                         << 1.0 * block_read_cpu_time / N_repetitions / 1e3
                         << std::endl;

    print_RDF_false_positive_rate_generic(db, system_verifier);
    testing_logger.output_statistics(testing_result_file, testing_result_file2,
                                     prefix);
    // print_perf_iostats_context(std::cout, 1);
  }
  // std::cout << prefix_number_of_PQs << " point_query_time_on_existing_keys_ns
  // = " << point_query_time_on_existing_keys_ns << std::endl;
  std::cout << prefix_number_of_PQs_on_existing_keys
            << " point_query_time_on_existing_keys_ns_out = "
            << point_query_time_on_existing_keys_ns << std::endl;
  ...
  testing_result_file << std::endl << std::endl;
  testing_result_file << "----------------------Testing On historic-existing "
                         "Keys-----------------------"
                      << std::endl;
  for (uint t = 0; t < system_verifier->getNumberOfRDFTypes(); t++) {
    long long total_read_count = 0;
    ...
    for (auto i = 0; i < N_repetitions; i++) {
      ...
      if (system_verifier->getStringOfRDFTypeChosed() ==
          "NONE_CACHE_RANGETOMBSTONE_TRACING") {
        string i_round_str = "i_round=" + std::to_string(i) + " ";
        // std::string prefix = " (Historcially Exist Keys " + i +
        // prefix_number_of_PQs + " " + i_round_str + ") " +
        // system_verifier->getStringOfRDFTypeChosed() + " ";
        std::string prefix = " (Historcially Exist Keys " + i +
                             prefix_number_of_PQs_on_historic_existing_keys +
                             " " + i_round_str + ") " +
    ...
    // if (number_of_PQs == -1){
    //   testing_result_file << "number of PQ = " <<
    //   system_verifier->getHistoricExistingKeys().size() << std::endl;
    // }else{
    //   testing_result_file << "number of PQ = " << number_of_PQs << std::endl;
    // }
    if (number_of_PQs_on_historic_existing_keys == -1) {
      testing_result_file << "number of PQ = "
                          << system_verifier->getHistoricExistingKeys().size()
                          << std::endl;
    } else {
      testing_result_file << "number of PQ = "
                          << number_of_PQs_on_historic_existing_keys
    ...
    // std::string prefix = " (Historcially Exist Keys) " +
    // system_verifier->getStringOfRDFTypeChosed() + " "; std::string prefix = "
    // (Historcially Exist Keys " + prefix_number_of_PQs + ") " +
    // system_verifier->getStringOfRDFTypeChosed() + " ";
    std::string prefix = " (Historcially Exist Keys " +
                         prefix_number_of_PQs_on_historic_existing_keys + ") " +
                         system_verifier->getStringOfRDFTypeChosed() + " ";
  ...
  testing_result_file << std::endl << std::endl;
  testing_result_file << "----------------------Testing On Currently Deleted "
                         "Keys-----------------------"
                      << std::endl;
  ...
  for (uint t = 0; t < system_verifier->getNumberOfRDFTypes(); t++) {
    ...
    for (auto i = 0; i < N_repetitions; i++) {
      ...
      if (flag_reopen_db_for_each_RDF_testing == true) {
        // retrieve_all_RDFs(db_ptr2, plrdf_prime, split_plrdf_prime,
        // top_level_rdf_prime, skyline_rdf_prime,
      ...
      if (system_verifier->getStringOfRDFTypeChosed() ==
          "NONE_CACHE_RANGETOMBSTONE_TRACING") {
        string i_round_str = "i_round=" + std::to_string(i) + " ";
        // std::string prefix = " (Currently Deleted Keys " + i +
        // prefix_number_of_PQs + " " + i_round_str + ") " +
        // system_verifier->getStringOfRDFTypeChosed() + " ";
        std::string prefix = " (Currently Deleted Keys " + i +
                             prefix_number_of_PQs_on_currently_deleted_keys +
      ...
      // std::string prefix = " (Currently Deleted Keys " + prefix_number_of_PQs
      // + ") " + system_verifier->getStringOfRDFTypeChosed() + " ";
      std::string prefix = " (Currently Deleted Keys " +
                           prefix_number_of_PQs_on_currently_deleted_keys +
                           ") " + system_verifier->getStringOfRDFTypeChosed() +
                           " ";
      testing_result_file2 << system_verifier->getMapPQTracingInfo(",", "\"",
                                                                   prefix, i)
                           << std::endl;
      testing_result_file2 << system_verifier->getVPQTracingInfo(",", "\"",
                                                                 prefix, i)
                           << std::endl;
      // system_verifier->clearMapPQTracingInfo();
      // system_verifier->clearVPQTracingInfo();
    ...
    // if (number_of_PQs == -1){
    //   testing_result_file << "number of PQ = " <<
    //   system_verifier->getCurrentlyDeletedKeys().size() << std::endl;
    // }else{
    //   testing_result_file << "number of PQ = " << number_of_PQs << std::endl;
    // }
    if (number_of_PQs_on_currently_deleted_keys == -1) {
      testing_result_file << "number of PQ = "
                          << system_verifier->getCurrentlyDeletedKeys().size()
                          << std::endl;
    } else {
      testing_result_file << "number of PQ = "
                          << number_of_PQs_on_currently_deleted_keys
  ...
  // std::cout << prefix_number_of_PQs << "
  // point_query_time_on_currently_deleted_all_ns = " <<
  // point_query_time_on_currently_deleted_all_ns << std::endl;
  std::cout << prefix_number_of_PQs_on_currently_deleted_keys
            << " point_query_time_on_currently_deleted_all_ns_out = "
            << point_query_time_on_currently_deleted_all_ns << std::endl;

  std::cout << "!!! Testing On Currently Non-inserted Keys " << std::endl;

  testing_result_file << std::endl << std::endl;
  testing_result_file << "----------------------Testing On Currently "
                         "Non-inserted Keys-----------------------"
                      << std::endl;
  int CurrentlyNonInsertedKeysNum =
      system_verifier->getCurrentlyNonInsertedKeysNum();
  testing_result_file << "number of currently non-inserted keys = "
                      << CurrentlyNonInsertedKeysNum << std::endl;
  ...
  for (uint t = 0; t < system_verifier->getNumberOfRDFTypes(); t++) {
    ...
    for (auto i = 0; i < N_repetitions; i++) {
      ...
      std::vector<uint64_t> cache_tombstone_bytes;
      //   for(auto &x: system_verifier->getCurrentlyNonInsertedKeys()){ // test
      //   on 1000 keys
      for (auto x : system_verifier->getCurrentlyNonInsertedKeysAtNRound(
               i)) {                                        // test on 1000 keys
      ...
      if (system_verifier->getStringOfRDFTypeChosed() ==
          "NONE_CACHE_RANGETOMBSTONE_TRACING") {
        string i_round_str = "i_round=" + std::to_string(i) + " ";
        // std::string prefix = " (Currently Non-Inserted Keys " + i +
        // prefix_number_of_PQs + " " + i_round_str + ") " +
        // system_verifier->getStringOfRDFTypeChosed() + " ";
        std::string prefix =
            " (Currently Non-Inserted Keys " + i +
            prefix_number_of_PQs_on_currently_non_inserted_keys + " " +
            i_round_str + ") " + system_verifier->getStringOfRDFTypeChosed() +
            " ";
        testing_result_file << ",\"" + prefix + " cache tombstone bytes\" : "
                            << "[";
        testing_result_file2 << ",\"" + prefix + " cache tombstone bytes\" : "
    ...
    // if (number_of_PQs == -1){
    //   testing_result_file << "number of PQ = " <<
    //   system_verifier->getCurrentlyNonInsertedKeys().size() << std::endl;
    // }else{
    //   testing_result_file << "number of PQ = " << number_of_PQs << std::endl;
    // }
    if (number_of_PQs_on_currently_non_inserted_keys == -1) {
    ...
    // std::string prefix = " (Non-inserted Keys " + prefix_number_of_PQs + ") "
    // + system_verifier->getStringOfRDFTypeChosed() + " ";
    std::string prefix = " (Non-inserted Keys " +
                         prefix_number_of_PQs_on_currently_non_inserted_keys +
                         ") " + system_verifier->getStringOfRDFTypeChosed() +
                         " ";
    testing_result_file2 << ",\"" + prefix + " elapsed time\" : "
