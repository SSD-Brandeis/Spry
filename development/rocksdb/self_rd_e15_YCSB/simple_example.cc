...
void print_perf_iostats_context(std::ostream& ofile, const std::string& prefix,
                                int N_repetitions = 1);
void init(DB** db_ptr2, Options& op, WriteOptions& write_op,
          ReadOptions& read_op, int max_background_jobs);
...
int main(int argc, char* argv[]) {
  ...
  if (parse_arguments2(argc, argv, _env, _plrdf_env, _surf_env,
                       system_verifier)) {
    exit(1);
  }
  ...
  // checking::SystemVerifier::getSystemVerifier()->setFlagUsingRocksdbDefaultValueOrFalseAsPrefetchIndexAndFilterInCacheDuringBlockBasedTableOpen(false);
  // std::this_thread::sleep_for(std::chrono::seconds(10));  // Sleep for 1
  // second
  // {
  ...
  //     // verification_runner::runPQVerification(db_ptr2, options, write_op,
  //     read_op, _env, numbers_of_PQs, kDBPath); int
  //     number_of_PQs_on_existing_keys = -1; int
  //     number_of_PQs_on_historic_existing_keys = -1; int
  //     number_of_PQs_on_currently_deleted_keys = -1; int
  //     number_of_PQs_on_currently_non_inserted_keys = -1;
  //     verification_runner::runPQVerification(db_ptr2, options, write_op,
  //     read_op, _env,
  //       number_of_PQs_on_existing_keys,
  //       number_of_PQs_on_historic_existing_keys,
  //       number_of_PQs_on_currently_deleted_keys,
  //       number_of_PQs_on_currently_non_inserted_keys, kDBPath);
  //   }
  //   {
  //     // int numbers_of_PQs = _env->number_of_PQ;
  //     // verification_runner::runPQVerification(db_ptr2, options, write_op,
  //     read_op, _env, numbers_of_PQs, kDBPath); int
  //     number_of_PQs_on_existing_keys = _env->number_of_PQs_on_existing_keys;
  //     int number_of_PQs_on_historic_existing_keys =
  //     _env->number_of_PQs_on_historic_existing_keys; int
  //     number_of_PQs_on_currently_deleted_keys =
  //     _env->number_of_PQs_on_currently_deleted_keys; int
  ...
  auto duration_all = std::chrono::duration_cast<std::chrono::nanoseconds>(
      stop_all - start_all);
  ...
  // plrdf_stringkey_prime, split_plrdf_stringkey_prime, top_level_rdf_prime,
  // top_level_rdf_stringkey_prime, skyline_rdf_prime);
