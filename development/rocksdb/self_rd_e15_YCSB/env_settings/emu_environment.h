...
class EmuEnv {
 ...
 public:
  ...
  uint16_t
      compaction_pri;  // c // 1 for kMinOverlappingRatio, 2 for
  ...
  uint16_t memtable_factory;    // 1 for skiplist, 2 for vector, 3 for hash
                                // skiplist, 4 for hash linklist

  // Compaction options
  long target_file_size_base;  // file size in level base (usually level 1)
  bool level_compaction_dynamic_level_bytes;  // related to BG-merging --
                                              // ensures last level is T times
                                              // larger than the previous level
  uint16_t
      compaction_style;  // 1 for kCompactionStyleLevel, 2 for
                         // kCompactionStyleUniversal, 3 for
                         // kCompactionStyleFIFO, 4 for kCompactionStyleNone
  bool disable_auto_compactions;        // for manual comapction (when
                                        // compactions_style = 4, i.e.,
                                        // kCompactionStyleNone)
  uint16_t compaction_filter;           // 0 for nullptr, 1 for invoking custom
                                        // compaction filter, if any
  uint16_t compaction_filter_factory;   // 0 for nullptr, 1 for invoking custom
                                        // compaction filter factory, if any
  int access_hint_on_compaction_start;  // 1: NONE, 2: NORMAL, 3: SEQUENTIAL, 4:
                                        // WILLNEED
                                        // https://linux.die.net/man/2/fadvise
  int level0_file_num_compaction_trigger;  // number of files that triggers
                                           // compaction in level0
  int target_file_size_multiplier;  // multiplier for file size if we want to
                                    // have same number of files per level
  int max_background_jobs;   // Maximum number of concurrent background jobs
                             // (compactions and flushes).
  int max_compaction_bytes;  // target_file_size_base * 25, limit number of
                             // bytes in one compaction to be lower than this
  ...
  int num_levels;   // Maximum number of levels that a tree may have
                    // [RDB_default: 7]
  ...
  bool cache_index_and_filter_blocks_with_high_priority;  // Deprecated by
                                                          // no_block_cache
  int read_amp_bytes_per_bit;      // temporarily 4; why 4 ?
  uint16_t data_block_index_type;  // 1 for kDataBlockBinarySearch, 2 for
                                   // kDataBlockBinaryAndHash
  uint16_t index_type;  // 1 for kHashSearch, 2 for kTwoLevelIndexSearch, 3 for
                        // kBinarySearchWithFirstKey
  bool partition_filters;
  int metadata_block_size;  // currently deprecated by data_block_index_type;
                            // TBC
  bool pin_top_level_index_and_filter;  // TBC
  uint16_t index_shortening;  // // {1: kNoShortening, 2: kShortenSeparators, 3:
                              // kShortenSeparatorsAndSuccessor}
  int block_size_deviation;   // TBC
  bool enable_index_compression;  // TBC

  // Compression options
  uint16_t compression;  // 1 for kNoCompression, 2 for kSnappyCompression, 3
                         // for kZlibCompression, 4 for kBZip2Compression, 5 for
                         // kLZ4Compression, 6 for kLZ4HCCompression, 7 for
                         // kXpressCompression, 8 for kZSTD, 9 for
                         // kZSTDNotFinalCompression, 10 for
                         // kDisableCompressionOption

  // ReadOptions
  bool verify_checksums;  // TBC
  bool fill_cache;  // data block/index block read for this iteration will not
                    // be cached
  int iter_start_seqnum;        // TBC
  bool ignore_range_deletions;  // TBC
  uint16_t read_tier;  // 1 for kReadAllTier, 2 for kBlockCacheTier, 3 for
                       // kPersistedTier, 4 for kMemtableTier

  // WriteOptions
  bool low_pri;  // every insert is less important than compaction
  bool sync;     // make every write wait for sync with log (so we see real perf
                 // impact of insert)
  bool disableWAL;
  ...
  // flushed op->db_write_buffer_size = 0;   // disabled op->arena_block_size =
  // 0; op->memtable_huge_page_size = 0; op.sample_for_compression = 0;    //
  // disabled op.bottommost_compression = kDisableCompressionOption;
