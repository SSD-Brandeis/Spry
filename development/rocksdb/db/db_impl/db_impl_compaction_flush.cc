...
Status DBImpl::BackgroundCompaction(bool* made_progress,
  ...
  *made_progress = false;
  ...
  if (!c) {
    ...
    std::cout << "compaction direct delete " << __FILE__ <<":" << __LINE__ << " " << __FUNCTION__ << std::endl;
    ...
    if(checking::SystemVerifier::getSystemVerifier()->hasRDFTypeOtherThanNone() == true){
      bool split_flag = false;
      c->column_family_data()->updateRDF2NewVersion(3, split_flag); // 1 for flush, 2 for compaction, 3 for compaction direcly deleted file
    }
    //yucheng Added End
    InstallSuperVersionAndScheduleWork(c->column_family_data(),
                                       &job_context->superversion_contexts[0],
  ...
  // } else if (!trivial_move_disallowed && c->IsTrivialMove() && false) { // <------------------- && false: do this only for top-level RDF behavioral simulation is included
        ...
        && (!checking::SystemVerifier::getSystemVerifier()->getSkipCompactionTrivialMove())
        ) { // <------------------- && false: do this only for top-level RDF behavioral simulation is included
    ...
    if(checking::SystemVerifier::getSystemVerifier()->hasRDFTypeOtherThanNone() == true){
      bool split_flag = false;
      c->column_family_data()->updateRDF2NewVersion(2, split_flag); // 1 for flush, 2 for compaction, 3 for compaction direcly deleted file
    }
    //yucheng Added End
    InstallSuperVersionAndScheduleWork(c->column_family_data(),
                                       &job_context->superversion_contexts[0],
