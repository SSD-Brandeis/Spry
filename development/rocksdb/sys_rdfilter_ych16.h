...
class PLRDF_t {
    ...
    for (uint64_t& file_num : file_numbers) {
      {
        ...
        if (it == rd_filter_level0.end()) {
          ...
          std::cerr << "File number not found in level 0 " << "File number "
                    << file_num << " " << __FILE__ << ":" << __LINE__
                    << std::endl;
          std::cerr << "Remindation: Do the manually flush after all the "
                       "insert workload are done. So no entries lie inside "
                       "memtable anymore. In case those entries will go "
                       "through the track of bulk buiding from WAL and no "
                       "going through the path of flushJob."
        ...
        auto val = it->second;
        to_be_added_in_next_level_rdf.insert(
            to_be_added_in_next_level_rdf.end(), val.begin(), val.end());
 ...
 public:
  ...
  void shiftRDFToOutputLevel(
      std::vector<std::tuple<int, int, std::vector<Pair>,
                             std::vector<uint64_t>>>* file_meta_data_vectors) {
    // init();
    // std::lock_guard<std::mutex> guard(update_mutex);
  ...
  bool isEntryAlive(uint level, KeyType key) {
    ...
    //[a,b), [c,d)
    // if(key >= it->first && key < it->second){return false;}
    // std::cout <<  " key = " << key << " range = " << it->first << "," <<
    // it->second << " " << __FILE__ << ":" << __LINE__ << std::endl; when keys
    // falling on the range boundary --> the result is uncertain for Pair keys
