...
key_size_to_insert_list = [16, 32, 64, 128, 256]
...
for i_test in range(5):
    ...
    rd = rd_list[0]
    sel = sel_list[0]
    params3["--key_size_to_insert"][0] = key_size_to_insert_list[i_test]
    ...
    if params3["--use_string_key"][0] == 0:
        log_dir = f"saved_result_int_key_size"
    else:
        log_dir = f"saved_result_string_key_size"

    log_dir = log_dir + f"/log_{i_test}_sel_{sel}_rd_{rd}_key_size_{params3['--key_size_to_insert'][0]}"
